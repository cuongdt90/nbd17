...
<div class="nbo-bulk-variation-wrap nbo-table-wrap">
    ...
    <table class="nbo-bulk-variation">
        ...
        <tbody>
            <tr ng-repeat="set in nbb_field_set track by $index">
                <td><input type="checkbox" class="nbo-bulk-checkbox" data-index="{{$index}}"></td>
            <?php 
                foreach($options["bulk_fields"] as $key => $bulk_index): 
                    $field = $options["fields"][$bulk_index];
            ?>
                <td>
                    ...
                    ?>
                    <input name="nbb-fields[{{$index}}][<?php echo $field['id']; ?>]" type="<?php echo $input_type; ?>"/>
                    <?php else: ?>
                    <select name="nbb-fields[{{$index}}][<?php echo $field['id']; ?>]" class="nbd-dropdown">
            ...
            <?php endforeach; ?>
                <td><input name="nbb-fields[][qty]" type="number" min="1" step="1" value="" required="" style="width: 4em" pattern="[0-9]*"/></td>
            </tr>            
        </tbody>
        <tfoot>
            <tr>
