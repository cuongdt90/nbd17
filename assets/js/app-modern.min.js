...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$timeout', '$http', '$document',
    ...
    $scope.loadGlobalTemplate = function(cid){
        ...
        $http({
            method: 'GET',
            url: appConfig.mediaUrl + '/template?limit=20&category=' + cid + '&search=' + search + '&start=' + ($scope.resource.globalTemplate.filter.currentPage-1) * 20
            ...
            function loadStage(stageIndex){
                ...
                function loadLayer(layerIndex){
                    ...
                    function fitTemplateWithStage(){
                        var maxSize = Math.max(stage.config.width, stage.config.height),
                        factor = maxSize / 200;
                        var selection = new fabric.ActiveSelection(stageLayers, {
                            canvas: _canvas
                        });
                        /* Move to center */
                        var left = selection.get('left'),
                        top = selection.get('top');
                        selection.set({
                            left: left + (maxSize - 200) / 2,
                            top: top + (maxSize - 200) / 2
                        });
                        _canvas.setActiveObject(selection);
                        /* zoom fit width stage */
                        var left = selection.get('left'),
                        top = selection.get('top');
                        selection.set({
                            scaleX: factor,
                            scaleY: factor,                            
                            left: left + 100 * (1 - factor),
                            top: top + 100 * (1 - factor)
                        });                        
