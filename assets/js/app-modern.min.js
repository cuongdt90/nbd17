...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$timeout', '$http', '$document',
    ...
    $scope.saveData = function(type){
        ...
        switch(type){
            ...
            default:
                ...
                $timeout(function(){
                    var action = ( NBDESIGNCONFIG.task == 'new' && NBDESIGNCONFIG.ui_mode == 2 ) ?  'nbd_save_cart_design' : 'nbd_save_customer_design';
                    NBDDataFactory.get(action, dataObj, function(data){
                        data = JSON.parse(data);
                        if(data.flag == 'success'){
                            if( type == 'saveforlater' || type == 'share' ){
    ...
    $scope.initStageSetting = function( id ){
        ...
        if( angular.isDefined($scope.settings.product_data.design) ){
            $scope.insertTemplate(true, {fonts: $scope.settings.product_data.fonts, design: $scope.settings.product_data.design});
        }
    };
    ...
    $scope.insertTemplate = function(local, temp){
        ...
        function loadDesign(fonts, design){
            function loadLayers(){ 
                _.each($scope.stages, function(stage, index){ 
...
//                            }, index);
//                        }else{
//                            $scope.loadTemplateLayer(layer, null, index);
//                        }
//                    }); 
            ...
            if(fonts.length){
                _.each(fonts, function(font, index){
                    ...
                    if( index == fonts.length - 1 ){
                        ...
                        $scope.insertTemplateFont(font.alias);
                    }
                });                
            }else{
                loadLayers();
        ...
        if( local ){
            loadDesign(temp.fonts, temp.design);
        }else{
...
//console.log(_canvas.getObjects());
    ...
    $scope.loadMyDesignInCart = function(){
