...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', '$timeout',function($scope, FabricWindow, $window, NBDDataFactory, filterFontFilter, $timeout){
    $scope.stages = [
        {
            config: {
    ...
    $scope.initSettings = function(){
        ...
        $scope.resource = {
            typography: {filter: {perPage: 20, currentPage: 1}, data: [], onload: false},
            font: {filter: {perPage: 10, currentPage: 1}, data: [], filteredFont : []}
        };
        ...
        //must todo with custom font
        $scope.resource.font.data = $scope.settings.gg_fonts;
    ...
    $scope.onEndRepeat = function(type){
        switch(type){
            case 'typography':
                var init = $scope.resource[type].filter.currentPage > 1 ? false : true;
    ...
    $scope.scrollLoadMore = function(container, type){
        if( type != 'font' ){
            if( $scope.resource[type].filter.currentPage * $scope.resource[type].filter.perPage > $scope.resource[type].data.length )  jQuery(container + ' .loading-photo').hide();
            if( $scope.resource[type].onload || $scope.resource[type].filter.currentPage * $scope.resource[type].filter.perPage > $scope.resource[type].data.length) return;
            jQuery(container + ' .loading-photo').show();
        }else{
            if( $scope.resource[type].filter.currentPage * $scope.resource[type].filter.perPage > $scope.resource[type].data.length) return;
        }
        if( $scope.resource[type].filter.currentPage * $scope.resource[type].filter.perPage < $scope.resource[type].data.length ){
    ...
    $scope.getFontInfo = function(alias){
        return _.filter($scope.fonts, { alias: alias })[0];
    };
    ...
    $scope.setTextAttribute = function(type, value, extra_option){
        ...
        switch(type){
            case 'is_uppercase':
                ...
                break;
            case 'fontFamily':
                if(!_.filter($scope.stages[$scope.currentStage].states.fontUsed, ['alias', extra_option.alias]).length){
                    $scope.stages[$scope.currentStage].states.fontUsed.push(extra_option);
                }
                break;
        }
        $scope.renderStage();
...
nbdApp.filter("filterFont", function() {
    return function(fonts, filterFont) {
        ...
        angular.forEach(fonts, function(font, key) {
            var check = [];
            check['limit'] = key > ( filterFont.perPage * filterFont.currentPage  - 1 ) ? false : true;
...
nbdApp.directive("fontOnLoad", [ function() {
    return {
        ...
        link: function(scope, element) {
            ...
            font.load(scope.preview, 10000).then(function () {
