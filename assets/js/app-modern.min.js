...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$timeout', '$http', 
    ...
    $scope.addImageFromUrl = function( url ){
        ...
        if(url.match(/\.(svg)$/) != null ){
            ...
            $scope.addArt(art, maybeHideLoading);
            return
        }
        ...
        NBDDataFactory.get('nbdesigner_copy_image_from_url', {url: url, gapi: $scope.resource.gapi}, function(data){
            ...
            if(data['flag'] == 1){
                $scope.addImage(data['src'], maybeHideLoading);      
            } else{
...
//        });
    ...
    $scope.addImage = function(url, maybeHideLoading){
        ...
        fabric.Image.fromURL(url, function(op) {
            ...
            if( maybeHideLoading ) $scope.toggleStageLoading();
        });        
    };
    /* SVG */
    ...
    $scope.addArt = function(art, maybeHideLoading){
        if(art.url.match(/\.(jpeg|jpg|gif|png)$/) != null){
            $scope.addImage(art.url, maybeHideLoading);
        }else{            
            fabric.loadSVGFromURL(art.url, function(ob, op) {
                $scope._addSvg(ob, op, art, maybeHideLoading);
            })            
        }
    };
    $scope._addSvg = function(ob, op, art, maybeHideLoading){
        var _canvas = $scope.stages[$scope.currentStage]['canvas'],
