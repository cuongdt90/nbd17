...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory',function($scope, FabricWindow, $window, NBDDataFactory){
    ...
    $scope.getTypography = function(){
        if( $scope.typography.length ) return;
    ...
    $scope.onEndRepeat = function(type){
        switch(type){
            case 'typography':
                var init = $scope.typographyLimit > 20 ? false : true;               
    ...
    $scope.scrollLoadMore = function(container, type){
        console.log('start load more: '+$scope.loading[type]);
        if( !$scope.loading[type] || $scope.typographyLimit > $scope.typography.length) return;
        jQuery(container + ' .loading-photo').show();
        switch(type){
            case 'typography':
                if( $scope.typographyLimit < $scope.typography.length ){
                    $scope.typographyLimit += 20;
...
//        console.log(item);
    ...
    $scope.stageToJson = function(stage_id){
        ...
        var json = this.stages[stage_id]['canvas'].toJSON(['itemId', 'selectable', 'lockMovementX', 'lockMovementY', 
            'lockScalingX', 'lockScalingY', 'lockRotation', 'rtl', 'elementUpload', 'forceLock', 'isBg', 
            'is_uppercase','available_color','available_color_list','color_link_group','isOverlay']);
        return json;
    };
    ...
    $scope.setStackLayerAlwaysOnTop = function(){
        //todo
    };
    ...
    $scope.changeOpacity = function(){
        ...
        items.forEach(function( item ){
            $scope.beforeObjectModify(item);
            item.set({opacity: $scope.stages[$scope.currentStage].states.opacity / 100});
        });
        $scope.renderStage();
    };
