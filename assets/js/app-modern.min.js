...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$timeout', '$http', '$document',
    ...
    $scope.initSettings = function(){
        ...
        if( $scope.resource.templates.length == 0 ){
    ...
    $scope.appConfig = {
        ...
        isModern: true,
        init: function(){
            this.mediaUrl = this.localMode ? 'http://localhost/nbmedia/v1' : 'https://media.printcart.com/v1';
        }
    };
    $scope.init = function() {
        $scope.appConfig.init();
    ...
    $scope.loadGlobalTemplate = function(cid){
        var search = '';
        $http({
            method: 'GET',
            url: $scope.appConfig.mediaUrl + '/template?limit=20&category=' + cid + '&search=' + search + '&start=' + ($scope.resource.globalTemplate.filter.currentPage-1) * 20
        ...
        $scope.getGlobalTemplate(id, function(data){
            ...
            if(fonts.length){
                _.each(fonts, function(font, index){
                    if(!_.filter($scope.resource.font.data, ['alias', font.alias]).length){console.log(font.alias);
            ...
            function loadStage(stageIndex){
                ...
                function loadLayer(layerIndex){
                    ...
                    function fitTemplateWithStage(){
                        var maxSize = Math.max(_canvas.width, _canvas.height),
                        factor = maxSize / 200;
                        ...
                        _.each(stageLayers, function(obj){
                            if( ['i-text', 'text', 'textbox', 'curvedText'].indexOf( obj.get('type') ) > -1  ){
...
nbdApp.factory('NBDDataFactory', function($http){
    return {
        get : function(action, data, callback) {
            ...
            if( data.source == 'media' ) url = 'http://localhost/nbmedia/v1/template';
