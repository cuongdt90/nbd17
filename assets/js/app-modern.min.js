...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$timeout', '$http', '$document',
    ...
    $scope.initStageSetting = function( id ){
        ...
        _canvas.calcOffset().requestRenderAll();
        $scope.updateApp();
    };
    $scope.initStagesSettingWithoutTemplate = function(){//return;
        _.each($scope.stages, function(stage, index){
            var _canvas = stage.canvas;
            ...
            function loadStage(stageIndex){
                ...
                function loadLayer(layerIndex){
                    ...
                    if( objects.length > 0 ){
                        ...
                        if( item.type == 'image' || item.type == 'custom-image' ){
                            fabric.Image.fromObject(item, function(_image){
                                _canvas.add(_image);
                                continueLoadLayer();
                            });
                        }else{
                            var klass = fabric.util.getKlass(item.type);
