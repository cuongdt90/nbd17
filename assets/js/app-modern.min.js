...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$timeout', '$http', 
    ...
    $scope.defaultStageStates = {
        ...
        itemId: null,
        tempParameters: null,
        usedFonts: [{name: 'Roboto', alias: 'Roboto'}],
        type: null,
        text: {
            fontFamily: {
                alias: 'Roboto',
    ...
    $scope.initSettings = function(){
        ...
        angular.extend($scope.settings, {
            ...
            showWarning: {status: false, type: 'oos'}
        });
    ...
    $scope.onClickPhotoTab = function(type){
        $scope.resource.photo.onclick = !$scope.resource.photo.onclick;
        if(!$scope.resource.photo.onclick) jQuery('#tab-photo .loading-photo').hide();
        switch(type){
            case 'dropbox':
            case 'facebook':
            case 'instagram':
            ...
            case 'Unsplash':
                ...
                if($scope.resource.photo.onclick && $scope.resource.photo.type != type){
                    $scope.getPhoto(type);
                }
                break;
            default:
    ...
    $scope.onEndRepeat = function(type){
        switch(type){
            ...
            case 'instagram':
                ...
                break;  
            case 'facebook':
                $scope.renderMasonryList(type, '#nbd-facebook-wrap .mansory-wrap', '.mansory-item', '#nbd-facebook-wrap', $scope.resource[type].init);
                break;     
            case 'upload':
                $scope.renderMasonryList(type, '#nbd-upload-wrap .mansory-wrap', '.mansory-item', '#nbd-upload-wrap', $scope.resource[type].init);
                break;             
        }
    };
            ...
            if( e.shiftKey ){
                ...
                switch( e.which ) {
                    ...
                    case 48:
                        ...
                        break;
                    case 49:
                        /* Hold Ctrl press 0 → Resize stage to origin size */
    ...
    $scope.calcViewport = function(){
        var _offset = checkMobileDevice() ? 20 : 100,
            _width = jQuery('.nbd-stages').width() - _offset,
            _height = jQuery('.nbd-stages').height() - _offset;
    ...
    $scope.precessProductSettings = function(){
        ...
        _.each($scope.settings.product_data.product, function(side, index){
            var _width = side.real_width * $scope.rateConvert2Px,
                _height = side.real_height * $scope.rateConvert2Px,
            ...
            if( object ){
                ...
                switch(object.type) {
                    ...
                    case 'curvedText':
                        ...
                        ['fontFamily', 'fontSize', 'textAlign', 'fontWeight', 'textDecoration', 'fontStyle', 'spacing', 'lineHeight', 'fill'].forEach(function(key){
    ...
    $scope.insertGoogleFont = function(font_name, callback){
        if(!_.filter($scope.resource.font.data, ['alias', font_name]).length){
            ...
            if( angular.isDefined(callback) ) callback(font_name);
        };
    };
    $scope._insertCanvaTypo = function(typo){
        //todo something
    };
    $scope.insertCanvaTypo = function(typo){
        if( $scope.settings.task != 'typography' ){
            $scope._insertCanvaTypo(typo);
            return;
        };
        $scope.zoomStage(0);
        ...
        fabric.Image.fromURL(url, function(op) {
            ...
            fabric.loadSVGFromURL(svg_url, function(ob, op) {
                ...
                $timeout(function(){
                    ...
                    object.set({dirty: true, selectable: false});
    ...
    $scope.toggleFullScreenMode = function(){
        ...
        var ele = document.getElementById('stage-container-' + $scope.currentStage);
...
//        });
    ...
    $scope.zoomStage = function(index){
        var _stage = this.stages[this.currentStage],
           _canvas = _stage['canvas'];
        _stage.states.currentScaleIndex = index;
        $scope.setStageDimension();
    ...
    $scope.selectAllLayers = function(){
        var _canvas = this.stages[this.currentStage]['canvas'];
        var selection = new fabric.ActiveSelection(_canvas.getObjects(), {
            canvas: _canvas
        });
    ...
    $scope.setTextAttribute = function(type, value, extra_option){
        ...
        switch(type){
            case 'is_uppercase':
                ...
                break;
            case 'fontFamily':
                if(!_.filter($scope.stages[$scope.currentStage].states.usedFonts, ['alias', value]).length){
                    $scope.stages[$scope.currentStage].states.usedFonts.push($scope.getFontInfo(value));
                }
    ...
    /* Preset: typography, preset design */
    ...
    $scope.addQrCode = function(){      
        ...
        jQuery('.main-qrcode').append(_qrcode);
    };    
    $scope.init();
}]);
