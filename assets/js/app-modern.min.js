...
//                                }
                            ...
                            if( angular.isDefined(item.avatar) ){
                                if( angular.isDefined($scope.settings.contact_sheet) ){
                                    item.src = $scope.settings.contact_sheet.avatar;
                                }
                            }
                            if( angular.isDefined(item.c_avatar) ){
                                if( angular.isDefined($scope.settings.contact_sheet) ){
                                    if( angular.isDefined($scope.settings.contact_sheet.contact[item.c_avatar]) ){
                                        item.src = $scope.settings.contact_sheet.contact[item.c_avatar].c_avatar;
                                    }else{
                                        mustInsert = false;
                                    }
                                }
                            ...
                            ['date', 'c_full_name', 'c_title', 'c_mobile', 'c_phone', 'c_email'].forEach(function (val) {
                                if (angular.isDefined(item[val])) {
                                    if (angular.isDefined($scope.settings.contact_sheet)) {
                                        if (val == 'date') {
                                            item.text = $scope.settings.contact_sheet[val];
                                        } else {
                                            if (angular.isDefined($scope.settings.contact_sheet.contact[item[val]])) {
                                                item.text = $scope.settings.contact_sheet.contact[item[val]][val];
                                            } else {
                                                mustInsert = false;
                                            }
                                        }
...
//        _canvas.add(rect);        
    ...
    $scope.setLayerAttribute = function(type, value, item_index, layer_index){
        ...
        if( !(itemType == 'i-text' || itemType == 'textbox') ){
            item.set({[type]: value});
        }
        switch(type){
            case 'visible':
            ...
            case 'selectable':
                ...
                if( $scope.isTemplateMode ){
                    var lock = angular.isDefined(item.forceLock) ? item.forceLock : false;
                    item.set({forceLock: !lock});
                }
                break;
            case 'lockScalingX':
    ...
    $scope.addContactSheet = function(index){
        if( index != 'contact' ){
            if( index != 'avatar' ){
                $scope.addText($scope.settings.contact_sheet[index], 'heading', {index: 1});
            }else{
                $scope.addImage($scope.settings.contact_sheet[index], true, true, index, 1);
            }
        }else{
            if( angular.isDefined($scope.settings.contact_sheet[index][$scope.currentContactIndex]) ){
                var contact = $scope.settings.contact_sheet[index][$scope.currentContactIndex];
                angular.forEach(contact, function(field, key){
                    if( key != 'c_avatar' ){
                        $scope.addText(field, 'heading', {key: $scope.currentContactIndex});
                    }else{
                        $scope.addImage(field, true, true, key, $scope.currentContactIndex);
                    }
                });
                $scope.currentContactIndex++;
            }
        }
    };
