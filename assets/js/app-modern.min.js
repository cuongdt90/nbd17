...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$timeout', '$http', '$document',
    ...
    $scope.initSettings = function(){
        ...
        $scope.resource = {
            ...
            element: {onclick: false, onload: false},
        ...
        $scope.includeExport = ['itemId', 'selectable', 'lockMovementX', 'lockMovementY','lockScalingX', 'lockScalingY', 'lockRotation', 'rtl', 'elementUpload', 'forceLock', 'isBg','is_uppercase','available_color','available_color_list','color_link_group','isOverlay', 'isAlwaysOnTop'];
    ...
    /* Extenal Image */
    $scope.addImageFromUrl = function( url ){
        if( url == '' ) return;
        $scope.toggleStageLoading();
    ...
    $scope.saveData = function(type){
        ...
        switch(type){
            case 'typography':
                ...
                break;
            default:
                dataObj.product_id = NBDESIGNCONFIG['product_id'];
                dataObj.variation_id = NBDESIGNCONFIG['variation_id'];
                dataObj.task = NBDESIGNCONFIG['task'];
                dataObj.task2 = NBDESIGNCONFIG['task2'];
                dataObj.design_type = NBDESIGNCONFIG['design_type'];
                dataObj.nbd_item_key = NBDESIGNCONFIG['nbd_item_key'];
                dataObj.cart_item_key = NBDESIGNCONFIG['cart_item_key'];
                dataObj.order_id = NBDESIGNCONFIG['order_id'];
                dataObj.enable_upload_without_design = NBDESIGNCONFIG['enable_upload_without_design'];
                dataObj.auto_add_to_cart = NBDESIGNCONFIG['auto_add_to_cart'];
                dataObj.used_font = new Blob([JSON.stringify($scope.resource.usedFonts)], {type: "application/json"}); 
                dataObj.design = new Blob([JSON.stringify($scope.resource.jsonDesign)], {type: "application/json"});
                ...
                $timeout(function(){
                    NBDDataFactory.get('nbd_save_customer_design', dataObj, function(data){
//                        _.each($scope.stages, function(stage, index){
...
//                        });
            ...
            if( e.shiftKey ){
                ...
                switch( e.which ) {
                    case 65:
                        ...
                        break;
                    case 66:
                        /* Hold Ctrl press B → set font weight */
                        if( _states.isText 
                    ...
                    case 80:
                        ...
                        break;                 
                    case 73:
                        /* Hold Ctrl press I → duplicate layers */
                        if( _states.isText 
                    ...
                    case 97:
                        ...
                        _states.fillScaleIndex != -1 && $scope.zoomStage(_states.fillScaleIndex) || $scope.zoomStage(_states.fitScaleIndex);
                        $scope.updateApp();                
                        break;
                    case 221:
    ...
    $scope.$on('canvas:created', function(event, id){
        ...
        if(!checkMobileDevice()) jQuery('#stage-container-'+id).perfectScrollbar();
    ...
    $scope.precessProductSettings = function(){
        ...
        _.each($scope.settings.product_data.product, function(side, index){
            ...
            $scope.stages[index].config.width = 400;
            $scope.stages[index].config.height = 18000;
    ...
    $scope.insertGoogleFont = function(font_name, callback){
        if(!_.filter($scope.resource.font.data, ['alias', font_name]).length){
            ...
            var font = new FontFaceObserver(fontName);
            font.load().then(function () {console.log(123);
                if( angular.isDefined(callback) ) callback(fontName);
            }, function () {
                console.log('Fail to load: '+fontName);
            });          
    ...
    $scope.insertTemplate = function(temp){
                ...
                _.each(layers, function(layer, _index){
                    if( _index == layers.length - 1 ){                                
                        $scope.loadTemplateLayer(layer, function(){                                    
                            $scope.renderTextAfterLoadFont(function(){
            ...
            if(fonts.length){
                _.each(fonts, function(font, index){
                    ...
                    if( index == fonts.length - 1 ){
                        $scope.insertGoogleFont(font.alias, function(){
                            loadLayers();
                        });
                    }else{
                        $scope.insertGoogleFont(font.alias);
                    }
                });
            }else{
                loadLayers();
    ...
    $scope.renderTextAfterLoadFont = function( callback ){
        _.each($scope.resource.tempData.template, function(item, index){
            ...
            if( index == ($scope.resource.tempData.template.length -1) && typeof callback == 'function' ){
                callback();
            } 
        })
    };
    $scope.loadTemplateLayer = function(json, callback){
        var stage = $scope.stages[$scope.currentStage],
        _canvas = stage['canvas'];        
                    ...
                    _.each(textIndexArr, function(el, index){
                        ...
                        if(el.type == 'text'){
                            ...
                            if( index < _.findLastIndex(textIndexArr, function(o) { return o.type == 'text'; }) ){
                                $scope.insertGoogleFont(el.font, function(fontName){
...
//console.log(_canvas.getObjects());
        ...
        _.each($scope.resource.font.data, function(font, index){
            var fontName = font.alias;
            var font_id = fontName.replace(/\s/gi, '').toLowerCase();
            if( !jQuery('#' + font_id).length ){
                jQuery('head').append('<link id="' + font_id + '" href="https://fonts.googleapis.com/css?family='+ fontName.replace(/\s/gi, '+') +':400,400i,700,700i" rel="stylesheet" type="text/css">');
            };
            var _font = new FontFaceObserver(fontName);
            _font.load($scope.settings.subsets[font.subset]['preview_text'], index * 3000).then(function () {
                console.log(index);
                $scope.contextAddLayers = 'template';
                var content = font.alias + ' ' + $scope.settings.subsets[font.subset]['preview_text'];
    ...
    $scope.addArt = function(art, showLoading, hideLoading){
        ...
        if(art.url.match(/\.(jpeg|jpg|gif|png)$/) != null){
            ...
            //fabric.loadSVGFromURL(art.url.replace(/^https:\/\//i, 'http://'), function(ob, op) {
