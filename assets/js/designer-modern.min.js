    ...
    canvas.on("object:selected", function(options) {
        ...
        if (object.type === "i-text" || object.type === "text" || object.type === "curvedText") {
            ...
            if( _.size(scope._allFonts )){
                ...
                if(!match) scope.currentFont = {alias : 'Tahoma', name : 'Tahoma'};
            }else {
                scope.currentFont = {alias : 'Tahoma', name : 'Tahoma'};
            }
    ...
    $scope.initStage = function(data){
        ...
        if( angular.isDefined(data.option) ) {
            ...
            if( angular.isDefined($scope.productOptions.multiple_edit) ){
                $scope.multipleEdit = $scope.productOptions.multiple_edit ? true : false;
            };
...
//        }
    ...
    $scope.saveDesign = function(orientation) {
        ...
        var json = canvas.toJSON(['itemId', 'selectable', 'lockMovementX', 'lockMovementY', 'lockScalingX', 'lockScalingY', 'lockRotation', 'rtl', 'elementUpload', 'forceLock', 'isBg', 'is_uppercase']);
        var config = {};
        if ($scope.modeMobile) {
            var _width = $scope.currentVariant.designArea.area_design_width * $scope.designScale,
                _height = $scope.currentVariant.designArea.area_design_height * $scope.designScale;
...
var NBDESIGNERCART = {
    build_form: function(data_img, data_json, fonts, config, action, svgs){
        ...
        _.each(svgs, function(val, key) {
            var svg = new Blob([val], {type: "application/octet-stream"});
...
initTooltip = function(){
    jQuery('.nbd-tooltip-i18n').each( function(){
        ...
        if( show_left_menu_tooltip ){
            ...
            title = langjs[key];
            if( key == 'ADD_TEXT' || key == 'ADD_CLIPART' || key == 'ADD_IMAGE' || key == 'FREE_DRAW' || key == 'ADD_QRCODE' ){
...
//      return;
//   }
//do something to check user design
};
...
(function ($) {
    ...
    var modern = {
        ...
        clickOutside: function () {
            $menuItem = $('.nbd-main-menu .menu-item,.nbd-dropdown');
            var $win = $(document);
        ...
        sideBar: {
            ...
            tabPhto: {
                ...
                main: function () {
                    var $tabPhoto = $('.nbd-sidebar #tab-photo');
                    var $tabScroll = $('.nbd-sidebar #tab-photo .tab-scroll');
                    var $resultLoaded = $('.nbd-sidebar #tab-photo .result-loaded');
                    var $gallery = $('.nbd-sidebar #nbdesigner-gallery');
                    var $mainCa = $('.nbd-sidebar #tab-photo .main-category');
                    var $category = $('.nbd-sidebar #tab-photo .categories .category');
                    var $categories = $('.nbd-sidebar #tab-photo .categories');
                    var $infoSupport = $('.nbd-sidebar #tab-photo .info-support');
                    var $typeCa = $('.nbd-sidebar #tab-photo .content-category .item-category');
                    var $loadingGif = $('.loading-photo');
                    var noCate = $category.length;
                    var noCateRow = parseInt(noCate / 3) + 1;

                    // set height for content cate
                    $categories.closest('.main-category').css({
                        'height': noCateRow * 132 + 'px'
                    });
                    modern.sideBar.tabPhto.initPositionCate();

                    // Event click in category
                    $category.on('click', function () {

                        // TODO load API image
                        var indexCa = $(this).index();
                        var indexCaRow = parseInt(indexCa / 3) + 1;
                        var sefl = this;
                        var widthCa = $(this).outerWidth();
                        var cateName = $(this).find('p').text();
                        var typeCa = $(this).attr('data-type');
                        $gallery.show();
                        switch (typeCa){
                            case 'image-upload':
                                // code here
                                $typeCa.hide();
                                $typeCa.filter('.type-upload').show();
                                $loadingGif.hide();
                                $gallery.hide();
                            ...
                            case 'facebook':
                                // code here
                                $typeCa.hide();
                                $typeCa.filter('.type-facebook').show();
                                $loadingGif.hide();
                                break;
                            case 'instagram':
                                // code here
                                $typeCa.hide();
                                $typeCa.filter('.type-instagram').show();
                                $loadingGif.hide();
                                $gallery.hide();
                                break;
                            case 'dropbox':
                                ...
                                $typeCa.hide();
                                $typeCa.filter('.type-dropbox').show();
                                $loadingGif.hide();
                                break;
                            case 'webcam':
                                // code here
                                ...
                                $typeCa.hide();
                                $loadingGif.hide();
                                // $typeCa.filter('.type-webcam').show();
                                $gallery.hide();
                        ...
                        $infoSupport.find('span').empty().append(cateName);
                        $category.filter('.active').removeClass('active');
                        if (!$(this).hasClass('active')) {
                            $(this).addClass('active');
                        }
                        $categories.parent().find('.pointer').css({
                            'left': ((widthCa) * (indexCa % 3 + 1) - widthCa / 2)  + 'px'
                        });

                        // Set height for categories
                        // var $mainCa = $category.closest('.main-category');
                        $mainCa.css({
                            'height': indexCaRow * 132 + 'px'
                        });
                        if (!$mainCa.hasClass('active-expanded')) {
                            var item = modern.sideBar.tabPhto.getImageItem();
                            $resultLoaded.show();
                            $gallery.empty();
                            for (var i=0;i<10;i++) {
                                $gallery.append(item);
                            ...
                            var nextAllCa = $('.nbd-sidebar #tab-photo .categories .category:nth-child(' + indexCaRow * 3 + ')').nextAll();
                            $(nextAllCa).each(function () {
                                $(this).hide();
                                // var positionTop = parseInt($(this).css('top'));
                                // $(this).css({
                                //     'top': positionTop + heightResult + 'px'
                                // });
                            ...
                            $gallery.imagesLoaded()
                                ...
                                setTimeout(function () {
                                    ...
                                    $mainCa.addClass('active-expanded');
                                    $resultLoaded.addClass('loaded');
                                    $loadingGif.hide();

                                    $gallery.masonry('destroy');
                                    var $gridLoaded = $gallery.masonry({
                                        itemSelector: '.nbdesigner-item'
                                    });
                                    $gallery.find('.nbdesigner-item').each(function (i) {
                            ...
                            $mainCa.removeClass('active-expanded');
                            $category.removeClass('slideInDown animated');
            ...
            tabSvg : function () {
                var $tabSvg = $('.nbd-sidebar .tab-content #tab-svg');
                nbdSvgDropdown.init();
        ...
        toolBar: {
            ...
            toolBarCommon: function () {
                ...
                var $addColorPalette = $('.nbd-color-palette .color-palette-add');
        ...
        animateStart: function () {
            ...
            $(window).on('load', function () {
                ...
                $mainBar.addClass('animated slideInDown');
    ...
    var nbdTemplateDropdown = {
        init: function () {
            this.main();
        },
        main: function () {
            var $items = $('.nbd-sidebar #tab-template .nbd-items-dropdown .items')
            var $item = $('.nbd-sidebar #tab-template .nbd-items-dropdown .items .item');
            var $mainItems = $('.nbd-sidebar #tab-template .nbd-items-dropdown .main-items');
            var $resultLoaded = $('.nbd-sidebar #tab-template .nbd-items-dropdown .result-loaded');
            var $galleryItem = $('.nbd-sidebar #tab-template .nbd-items-dropdown .nbdesigner-gallery');
            var noItem = $item.length;
            var noItemRow = parseInt(noItem / 2);
            var itemHeight = $item.outerHeight() + 10;
            var $loadingGif = $('.nbd-sidebar #tab-template .nbd-items-dropdown .loading-photo');
            var isMasonry = false;

            nbdTemplateDropdown.initPositionItem($items, $item);
            // set height for content cate
            $mainItems.css({
            ...
            $item.on('click', function () {
                ...
                var indexItemRow = parseInt(indexItem / 2) + 1;
                ...
                if (!$mainItems.hasClass('active-expanded')) {
                    ...
                    var item = nbdTemplateDropdown.getImageItem();

                    for (var i=0;i<10;i++) {
                        $galleryItem.append(item);
                    }

                    var nextAllItem = $('.nbd-sidebar #tab-template .nbd-items-dropdown .items .item:nth-child(' + indexItemRow * 2 + ')').nextAll();
        ...
        initPositionItem: function (items,item) {
            // TODO check has el
            var leftItem = items.width() / 2;
            var topItem = item.height() + 20;
            item.show();
            item.each(function () {
                var index = $(this).index();
                var indexMod = index % 2;
                var indexI = parseInt(index / 2);
                $(this).css({
        ...
        getImageItem: function () {
            var src = 'http://demo2.cmsmart.net/web2print/wp-content/uploads/nbdesigner/designs/610bd64612/preview/frame_1.png';
    ...
    var nbdSvgDropdown = {
        ...
        main: function () {
            var $items = $('.nbd-sidebar #tab-svg .nbd-items-dropdown .items');
            var $item = $('.nbd-sidebar #tab-svg .nbd-items-dropdown .items .item');
            var $mainItems = $('.nbd-sidebar #tab-svg .nbd-items-dropdown .main-items');
            var $resultLoaded = $('.nbd-sidebar #tab-svg .nbd-items-dropdown .result-loaded');
            var $galleryItem = $('.nbd-sidebar #tab-svg .nbd-items-dropdown .nbdesigner-gallery');
            var $infoSupport = $('.nbd-sidebar #tab-svg .info-support');
            var $tabScroll = $('.nbd-sidebar #tab-svg .tab-scroll');
            var noItem = $item.length;
            var noItemRow = parseInt(noItem / 3);
            var itemHeight = $item.outerHeight() + 10;
            var $loadingGif = $('.nbd-sidebar #tab-svg .nbd-items-dropdown .loading-photo');
            var isMasonry = false;

            nbdSvgDropdown.initPositionItem($items, $item);
            // set height for content cate
            $mainItems.css({
                'height': noItemRow * itemHeight + 'px'
            });
            $item.on('click', function () {
                var indexItem = $(this).index();
                var indexItemRow = parseInt(indexItem / 3) + 1;
                ...
                $mainItems.find('.pointer').css({
                    'left': ((widthItem) * (indexItem % 3 + 1) - widthItem / 2)  + 'px'
                });
                ...
                if (!$mainItems.hasClass('active-expanded')) {
                    ...
                    var item = nbdSvgDropdown.getImageItem();
                    for (var i=0;i<30;i++) {
                        $galleryItem.append(item);
                    }

                    var nextAllItem = $('.nbd-sidebar #tab-svg .nbd-items-dropdown .items .item:nth-child(' + indexItemRow * 3 + ')').nextAll();
                    $(nextAllItem).each(function () {
                        $(this).hide();
                        // var positionTop = parseInt($(this).css('top'));
                        // $(this).css({
        ...
        initPositionItem: function (items,item) {
            // TODO check has el
            var leftItem = items.width() / 3;
            var topItem = item.height() + 10;
            item.show();
            item.each(function () {
                var index = $(this).index();
                var indexMod = index % 3;
        ...
        getImageItem: function () {
            var src = 'http://dev.cmsmart.net:3000/wp46/wp-content/uploads/2016/10/doghead1.svg';
            var src2 = 'http://dev.cmsmart.net:3000/wp46/wp-content/uploads/2016/10/silhouette_fox-1.svg';
            var src3 = 'http://dev.cmsmart.net:3000/wp46/wp-content/uploads/2016/10/seagull_contour.svg';
            var src4 = 'http://dev.cmsmart.net:3000/wp46/wp-content/uploads/2016/10/silhouette_chipmunk.svg';
            var src5 = 'http://dev.cmsmart.net:3000/wp46/wp-content/uploads/2016/10/silhouette_kangaroo.svg';
            var src6= 'http://dev.cmsmart.net:3000/wp46/wp-content/uploads/2016/10/doghead.svg';
            var src7 = 'http://dev.cmsmart.net:3000/wp46/wp-content/uploads/2016/10/greedy-rat-1.svg';
            var src9 = 'http://dev.cmsmart.net:3000/wp46/wp-content/uploads/2016/10/silhouette_beaver-1.svg';
            var src8 = 'http://dev.cmsmart.net:3000/wp46/wp-content/uploads/2016/10/silhouette_giraffe.svg';
            var item = '<div class="nbdesigner-item"><img src="' + src + '"></div>'
                + '<div class="nbdesigner-item"><img src="' + src2 + '"></div>'
                + '<div class="nbdesigner-item"><img src="' + src3 + '"></div>'
                + '<div class="nbdesigner-item"><img src="' + src4 + '"></div>'
                + '<div class="nbdesigner-item"><img src="' + src5 + '"></div>'
                + '<div class="nbdesigner-item"><img src="' + src6 + '"></div>'
                + '<div class="nbdesigner-item"><img src="' + src7 + '"></div>'
                + '<div class="nbdesigner-item"><img src="' + src8 + '"></div>'
                + '<div class="nbdesigner-item"><img src="' + src9 + '"></div>';
            return item;
        },
    };
    var nbdElementDropdown = {
        init: function () {
            this.main();
        },
        main: function () {
            var $items = $('.nbd-sidebar #tab-element .nbd-items-dropdown .items')
            var $item = $('.nbd-sidebar #tab-element .nbd-items-dropdown .items .item');
            var $mainItems = $('.nbd-sidebar #tab-element .nbd-items-dropdown .main-items');
            var $resultLoaded = $('.nbd-sidebar #tab-element .nbd-items-dropdown .result-loaded');
            var $galleryItem = $('.nbd-sidebar #tab-element .nbd-items-dropdown .nbdesigner-gallery');
            var $tabScroll = $('.nbd-sidebar #tab-element .tab-scroll');
            var $contentItem = $('.nbd-sidebar #tab-element .result-loaded .content-item');
            var $loadingGif = $('.nbd-sidebar #tab-element .nbd-items-dropdown .loading-photo');
            var $infoSupport = $('.nbd-sidebar #tab-element .info-support');
            var isMasonry = false;
            ...
            nbdElementDropdown.initPositionItem($items, $item);
            ...
            $item.on('click', function () {
                ...
                switch (dataType) {
                    case 'draw':
                        ...
                        $contentItem.filter('.type-draw').show();
                        $galleryItem.hide();

                        if (!$mainItems.hasClass('active-expanded')) {
                            $mainItems.addClass('active-expanded');
                            var nextAllItem = $('.nbd-sidebar #tab-element .nbd-items-dropdown .items .item:nth-child(' + indexItemRow * 3 + ')').nextAll();
                            $(nextAllItem).each(function () {
                                $(this).hide();
                            });
                            $resultLoaded.addClass('loaded');
                    ...
                    case 'shapes':
                        ...
                        if (!$mainItems.hasClass('active-expanded')) {
                            ...
                            var item = nbdGetItems.getImageItemElementShapes();
                            for (var i=0;i<30;i++) {
                                $galleryItem.append(item);
                            }
                            $resultLoaded.show();
                    ...
                    case 'qr-code':
                        //code hear
                        $resultLoaded.show().removeClass('overflow-visible');
                        $contentItem.filter('.type-qrcode').show();
                        $galleryItem.hide();
    ...
    var nbdGetItems = {
        getImageItemTypography: function () {
            var src = 'http://192.168.9.175/online-design/wp-content/plugins/web-to-print-online-designer/data/typography/img/2.png';
            var item = '<li class="typography-item"><img src="' + src + '"></li>';
            return item;
        },
        getImageItemElementShapes: function () {
    ...
    $.fn.nbdDropdown = function (options) {
        ...
        return this.each(function () {
            ...
            var $tabScroll = $(this).find('.tab-scroll');
            ...
            $item.on('click', function () {
                ...
                if (dataApi == 'false') {
                    ...
                    if (!$mainItems.hasClass('active-expanded')) {
                        $mainItems.addClass('active-expanded');
                        $resultLoaded.addClass('loaded');
                        var nextAllItem = $items.find('.item:nth-child(' + indexItemRow * 3 + ')').nextAll();
                        $(nextAllItem).each(function () {
                            $(this).hide();
                        });
                    }else {
                ...
                if (!$mainItems.hasClass('active-expanded')) {
                    ...
                    var item = nbdSvgDropdown.getImageItem();
                    item = sefl.getApiElement(dataType);
                    for (var i=0;i<30;i++) {
                        $galleryItem.append(item);
                    }
                    var nextAllItem = $('.nbd-sidebar #tab-svg .nbd-items-dropdown .items .item:nth-child(' + indexItemRow * 3 + ')').nextAll();
    ...
    $(document).ready(function () {
        ...
        $('#tab-element-test .nbd-items-dropdown').nbdDropdown({
            'getServer': {
                'animal' : 'aaaaa',
            },
            'itemInRow': 3,
