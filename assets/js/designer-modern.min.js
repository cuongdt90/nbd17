...
//                        $resultLoaded.find('.nbdesigner-gallery').empty();
    ...
    $.fn.nbdTour = function (obj) {
        var defaults = {};
        var opts = $.extend({}, $.fn.nbdTour.defaults, obj);
        return this.each(function () {
            var $tourActive = $(this), $bgTour = $('.nbd-bgTour'), $infoTour = $('.nbd-tourStep');
            if ($tourActive.offset().top < 0) {
                $tourActive.closest('.tab-scroll').scrollTop(0);
            }
            // trigger
            if (typeof opts.seflTrigger !== 'undefined') {
                if (opts.seflTrigger == true) $tourActive.triggerHandler('click');
            }
            if (typeof opts.parentTrigger !== 'undefined') {
                $tourActive.closest('.tab').addClass('nbd-notransition');
                $(opts.parentTrigger).triggerHandler('click');
            }
            // $tourActive.trigger('click');
            if (typeof opts.trigger !== 'undefined') {
                $(opts.trigger.el).triggerHandler(opts.trigger.ev);
            }

            var height = $tourActive.outerHeight(), width = $tourActive.outerWidth(), offset = $tourActive.offset(),
                top = offset.top, left = offset.left, bottom = $(window).height() - height -top,
                right = $(window).width() - left - width;

            // tour extra
            $('.nbd-tourStep-extra').removeClass('nbd-tourStep-extra');
            if (typeof opts.extra !== 'undefined') {
                $(opts.extra.el).addClass('nbd-tourStep-extra');
            }

            $infoTour.find('.tour-title').text(opts.title);
            $infoTour.find('.tour-intro').text(opts.desc);

            $bgTour.filter('.bgTour-top').css({
                height: top + 'px'
            });
            $bgTour.filter('.bgTour-bottom').css({
                height: bottom + 'px'
            });
            $bgTour.filter('.bgTour-right').css({
                top: top + 'px',
                left: left + width + 'px',
                height: height + 'px'
            });
            $bgTour.filter('.bgTour-left').css({
                top: top + 'px',
                width: left + 'px',
                height: height + 'px'
            });
            ...
            if ($(window).width() > (left * 2)) {
                $infoTour.attr('data-pos', 'left');
                $infoTour.css({
                    top: top + 'px',
                    left: left + width + 7 + 'px'
                });
            }else {
                $infoTour.attr('data-pos', 'right');
                $infoTour.css({
                    top: top + 'px',
                    left: left - 7 + 'px'
                });
    ...
    function nbdTourStart(objTour) {
        var bgTour = '<div class="nbd-bgTour bgTour-top"></div>' +
            '<div class="nbd-bgTour bgTour-bottom"></div>' +
            '<div class="nbd-bgTour bgTour-left"></div>' +
            '<div class="nbd-bgTour bgTour-right"></div>';
        var infoTour = '<div class="nbd-tourStep nbd-show nbd-triangle">' +
                '<div class="main-tour">' +
                    '<i class="icon-nbd icon-nbd-clear tour-close js-tour-close"></i>' +
                    '<div class="tour-padding">' +
                        '<div class="tour-title">Shapes</div>' +
                        '<div class="tour-intro">Choose from library of different shapes which are useful to create your design; Color chart is also available to change color of shape</div>' +
                    '</div>' +
                    '<div class="tour-footer">' +
                        '<div class="tour-pagination">' +
                            '<div class="item-pag tour-prev">prev</div>' +
                            '<div class="item-pag tour-count"><span class="step-cur">1</span> of <span class="step-total">6</span></div>' +
                            '<div class="item-pag tour-next">Next</div>' +
                        '</div>' +
                    '</div>' +
                '</div>' +
            '</div>';
        $('.nbd-mode-modern').append(bgTour);
        $('.nbd-mode-modern').append(infoTour);
        var $tours = $('[data-tour]'), $tourActive = null, $bgTour = $('.nbd-bgTour'), $infoTour = $('.nbd-tourStep'),
            obj = {desc: '',title: '', trigger: {}, extra: {}}, step = 1, $tourActive = null;

        // init
        $tourActive = $tours.filter($('[data-tour-active]'));
        step = $tourActive.data('tour-step');
        if (typeof $tourActive == null) {
            $tourActive = $tours.first();
            step = 1;
        }
        obj = objTour[step];
        if (typeof objTour[step].title !== 'undefined' && objTour[step].title !== '') {
            obj.title = objTour[step].title;
        }else if (typeof $tourActive.data('title') !== 'undefined') {
            obj.title = $tourActive.data('title');
        }else{
            obj.title = 'Tour Guide';
        }
        $tourActive.nbdTour(obj);

        if (step == 1) $infoTour.find('.tour-prev').empty();
        $infoTour.find('.step-cur').text(step);
        $infoTour.find('.step-total').text(Object.keys(objTour).length);

        $infoTour.find('.tour-next').on('click', function () {
            step += 1;
            if (step > Object.keys(objTour).length) {
                $('.js-tour-close').triggerHandler('click');
                return;
            }
            if (step == Object.keys(objTour).length) {
                $(this).text('Finish');
                $(this).addClass('tour-finish');
            }
            $infoTour.find('.tour-prev').text('Prev');
            $tourActive = $tours.filter(function () {
                return $(this).data('tour-step') == step;
            });
            $infoTour.find('.step-cur').text(step);
            obj = objTour[step];
            if (typeof objTour[step].title !== 'undefined' && objTour[step].title !== '') {
        ...
        $infoTour.find('.tour-prev').on('click', function () {
            step -= 1;
            if (step < 1) {
                $('.js-tour-close').triggerHandler('click');
                return;
            }
            if (step == (Object.keys(objTour).length - 1)) {
                $infoTour.find('.tour-next').text('Next');
            }
            if (step == 1) {
                $(this).empty();
            }
        ...
        $infoTour.find('.js-tour-close').on('click', function () {
            $bgTour.remove();
            $infoTour.remove();
            $('.nbd-tourStep-extra').removeClass('nbd-tourStep-extra');
            $('.nbd-notransition').removeClass('nbd-notransition');
        });
        ...
        $(document).keypress(function(e) {
            ...
            if (e.shiftKey) {
                if(e.which == 13) {
                    $infoTour.find('.tour-prev').triggerHandler('click');
                }
            }else {
                if(e.which == 13) {
                    $infoTour.find('.tour-next').triggerHandler('click');
                }
            }
        });
    };
...
//        });
        ...
        var objTour = {
            1: {
                desc: 'Choose from library of different shapes which are useful to create your design',
                selector: '.tabs-nav #tab-typo',
                seflTrigger: true,
            },
            2: {
                desc: 'Text heading',
                selector: '.nbd-sidebar .typography-head .text-heading',
                seflTrigger: true,
                parentTrigger: '.tabs-nav #tab-typo',
                trigger: {
                    el: '.nbd-sidebar .typography-head .text-heading',
                    ev: 'click'
                },
                extra: {
                    el: '.nbd-workspace .stage-main, .nbd-toolbar'
                }

            },
            3: {
                desc: 'Choose from library of different shapes which are useful to create your design',
                selector: '.nbd-sidebar .typography-head .text-sub-heading',
                parentTrigger: '.tabs-nav #tab-typo',
                trigger: {
                    el: '.nbd-sidebar .typography-head .text-sub-heading',
                    ev: 'click'
                },
                extra: {
                    el: '.nbd-workspace .stage-main, .nbd-toolbar'
            ...
            4: {
                desc: 'Choose from library of different shapes which are useful to create your design',
                selector: '.tabs-nav .nav-tab-clipart',
                seflTrigger: true,
            },
            5: {
                desc: 'Choose from library of different shapes which are useful to create your design',
                selector: '.tabs-nav .nav-tab-photos',
                seflTrigger: true,
            },
            6: {
                desc: 'Choose from library of different shapes which are useful to create your design',
                selector: '.tabs-nav .nav-tab-elements',
                seflTrigger: true,
            },
            7: {
                desc: 'Choose from library of different shapes which are useful to create your design',
                selector: '.tabs-nav .nav-tab-layers',
                seflTrigger: true,
            },
            8:{
               desc: 'xxxx',
                selector: '.nbd-main-bar .menu-item.item-process',
                title: 'Process'
            }
        }
        // add attibute to el
        _.forEach(objTour, function (value, key) {
            $(value.selector).attr('data-tour', '');
            $(value.selector).attr('data-tour-step', key);
        });
        $('[data-tour]').on('click', function () {
            $('[data-tour]').removeAttr('data-tour-active');
            $(this).attr('data-tour-active', '');
        });

        $('.tour-start').on('click', function () {
            nbdTourStart(objTour);
            nbdTourKeypress();
        });
