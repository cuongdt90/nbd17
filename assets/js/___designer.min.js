...
{};$scope.undos={};$scope.redos={};$scope.dataCustomerDesign={};$scope.editable={};$scope._=_;$scope.uploadURL=[];$scope.file=[];$scope.loading=false;$scope.currentCateFont="Font Categories";$scope.contentText="Text Here";$scope.designScale=calcCanvasWidth()/300;$scope.scale={};$scope.rateConvertCm2Px=37.8;$scope.currentScale="1";$scope.iid="images";$scope.qrCodeContent="http://example.com/";$scope.imgPageSize=10;$scope.artPageSize=10;$scope.artPageSizes=[];$scope.imgCurrentPage=0;$scope.artCat=[];
$scope.arts=[];$scope.curentCatArt="0";$scope.currentCatArtName="Categories";$scope.editable.paths=[];$scope.pathColor=[];$scope.countArt=0;$scope.fontPageSize=10;$scope.fontPageSizes=[];$scope.fontCat=[];$scope.fonts=[];$scope.AllFonts=[];$scope.google_font=[];$scope.googleFont=[];$scope.customFont=[];$scope.googleFontPageSize=10;$scope.adminTemplatePageSize=8;$scope.curentCatFont="0";$scope.currentCatFontName="Categories";$scope.countFont=0;$scope.enableGroup=false;$scope.colorBrush="cc324b";$scope.brushShadowWidth=
1;$scope.brushWidth=1;$scope.brushOpacity=1;$scope.brushAngle=0;$scope.colorShape="f98332";$scope.shapeMode=false;$scope.currentPreview=od_mainURL+"images/default.png";$scope.processing=100;$scope.currentPathCrop=listPath.path_crop;$scope.patterns=[];$scope.patternPageSize=10;$scope.imageFromUrl="";$scope.fontCollection={};$scope.currentFontCollection=[];$scope.ajustmentScale=1;$scope.ajustmentOrientationScale=[];$scope.flagChangeOrientation=[];$scope.ignoreLocalDesign=false;$scope.adminListTemplate=
[];$scope.backgroundId=0;$scope.statusWebcam=false;$scope.onLoadProduct=false;$scope.rtlLang=false;$scope.langMode=NBDESIGNCONFIG["lang_rtl"];$scope.refProduct=[];$scope.showAlignToolbar=false;$scope.holdShiftKey=false;$scope.gridMode=false;$scope.task=OD_task;$scope.orientationActiveUndoStatus=false;$scope.orientationActiveRedoStatus=false;$scope.currentItemParameters={};$scope.readyReplaceImage=false;$scope.resetDimension=function(){$scope.designerWidth=calcCanvasWidth();$scope.designerHeight=calcCanvasWidth();
...
1;$scope.canvas.setActiveObject($scope.canvas.item(index));$scope.canvas.item($scope.editableItem).centerH();$scope.canvas.item($scope.editableItem).centerV();$scope.canvas.item($scope.editableItem).setCoords();$scope.canvas.item($scope.editableItem).set({"itemId":itemId});$scope.drawCanvas();$scope.setUndoRedo({element:$scope.canvas.item(index),parameters:$scope.getItemJson($scope.canvas.item(index)),interaction:"add"});$scope.addLayers($scope.canvas.getActiveObject(),type,index)};$scope.setItemConfig=
function(item,prop,value){if(!!item.configuration){var config=item.configuration;if(config.hasOwnProperty(prop)){config[prop]=value;item.configuration=config}else{var con={};con[prop]=value;item.configuration=_.merge(config,con)}}else{var config={};config[prop]=value;item.configuration=config}};$scope.getItemConfig=function(item,prop){if(!!item.configuration){var config=item.configuration;if(config.hasOwnProperty(prop))return config[prop];else return false}else return false};$scope.addText=function(){$scope.disableDrawMode();
$scope.showPopover("text");$scope.zoom=1;$scope.zoomCanvas();var text=new fabric.Text($scope.contentText,{left:100,top:10,fill:"#cc324b",fontFamily:"Arial",radius:50,fontSize:30,textAlign:"center",spacing:0});$scope.canvas.add(text);$scope.ajustAfterAddItem("text")};$scope.ChangeText=function(){if($scope.editableItem==null)return;$scope.canvas.item($scope.editableItem).setText($scope.editable.text);var type=$scope.canvas.item($scope.editableItem).get("type");$scope.addLayers($scope.canvas.item($scope.editableItem),
type,$scope.editableItem);$scope.drawCanvas()};$scope.changeFont=function(font){if($scope.editableItem==null)return;var stop=$interval(function(){var name="";if(font.alias)name=font.alias;else name=font.name;if(isFontAvailable(name)){$scope.setUndoRedo({element:$scope.canvas.item($scope.editableItem),parameters:JSON.stringify({"fontFamily":$scope.canvas.item($scope.editableItem).fontFamily}),interaction:"modify"});$scope.currentFont=font;$scope.canvas.item($scope.editableItem).setFontFamily(name);
$scope.canvas.item($scope.editableItem).set("fontName",name);var index=$scope.canvas.getObjects().length-1;$scope.canvas.setActiveObject($scope.canvas.item($scope.editableItem));$scope.canvas.item($scope.editableItem).setCoords();$scope.drawCanvas();$interval.cancel(stop)}},100);$scope.currentFontCollection[$scope.editableItem]=font;jQuery("#dg-fonts").modal("hide")};$scope.addTeam=function(){$scope.showPopover("team")};$scope.closePopover=function(){$scope.Popover="none"};$scope.showPopover=function(type){$scope.pop.type=
type;$scope.pop.text="none";$scope.pop.art="none";$scope.pop.qrcode="none";$scope.pop.clipArt="none";$scope.pop.draw="none";$scope.Popover="block";switch(type){case "text":$scope.pop.text="block";break;case "art":$scope.pop.art="block";break;case "qrcode":$scope.pop.qrcode="block";break;case "clipArt":$scope.pop.clipArt="block";break;case "draw":$scope.pop.draw="block";break;default:$scope.pop.text="none";$scope.pop.art="none";$scope.pop.qrcode="none";$scope.pop.clipArt="none";$scope.pop.draw="none"}};
$scope.drawCanvas=function(){$scope.canvas.calcOffset();$scope.canvas.renderAll()};$scope.changeColor=function(e){if($scope.editableItem==null)return;$scope.setUndoRedo({element:$scope.canvas.item($scope.editableItem),parameters:JSON.stringify({"fill":$scope.editable.fill}),interaction:"modify"});var color="#"+e;$scope.canvas.item($scope.editableItem).setFill(color);$scope.editable.fill=color;$scope.drawCanvas()};$scope.changeStroke=function(c,stroke){if($scope.editableItem==null)return;$scope.setUndoRedo({element:$scope.canvas.item($scope.editableItem),
parameters:JSON.stringify({"stroke":$scope.editable.stroke}),interaction:"modify"});if(c){var color="#"+c;$scope.editable.stroke=color;$scope.canvas.item($scope.editableItem).setStroke($scope.editable.stroke)}if(stroke)$scope.canvas.item($scope.editableItem).setStrokeWidth(stroke);$scope.drawCanvas()};$scope.toggleItalic=function(){if($scope.editableItem==null)return;$scope.setUndoRedo({element:$scope.canvas.item($scope.editableItem),parameters:JSON.stringify({"fontStyle":$scope.canvas.item($scope.editableItem).fontStyle}),
...
if(obj==null)return;$scope.setUndoRedo({element:$scope.canvas.item($scope.editableItem),parameters:JSON.stringify({"left":$scope.canvas.item($scope.editableItem).left}),interaction:"modify"});var width=$scope.canvas.width/100;var left=$scope.canvas.item($scope.editableItem).left;$scope.canvas.item($scope.editableItem).set("left",left-width);$scope.canvas.item($scope.editableItem).setCoords();$scope.drawCanvas()};$scope.ShiftRight=function(){var obj=$scope.canvas.getActiveObject();if(obj==null)return;
$scope.setUndoRedo({element:$scope.canvas.item($scope.editableItem),parameters:JSON.stringify({"left":$scope.canvas.item($scope.editableItem).left}),interaction:"modify"});var width=$scope.canvas.width/100;var left=$scope.canvas.item($scope.editableItem).left;$scope.canvas.item($scope.editableItem).set("left",left+width);$scope.canvas.item($scope.editableItem).setCoords();$scope.drawCanvas()};$scope.ShiftUp=function(){var obj=$scope.canvas.getActiveObject();if(obj==null)return;$scope.setUndoRedo({element:$scope.canvas.item($scope.editableItem),
parameters:JSON.stringify({"top":$scope.canvas.item($scope.editableItem).top}),interaction:"modify"});var height=$scope.canvas.height/100;var top=$scope.canvas.item($scope.editableItem).top;$scope.canvas.item($scope.editableItem).set("top",top-height);$scope.canvas.item($scope.editableItem).setCoords();$scope.drawCanvas()};$scope.ShiftDown=function(){var obj=$scope.canvas.getActiveObject();if(obj==null)return;$scope.setUndoRedo({element:$scope.canvas.item($scope.editableItem),parameters:JSON.stringify({"top":$scope.canvas.item($scope.editableItem).top}),
interaction:"modify"});var height=$scope.canvas.height/100;var top=$scope.canvas.item($scope.editableItem).top;$scope.canvas.item($scope.editableItem).set("top",top+height);$scope.canvas.item($scope.editableItem).setCoords();$scope.drawCanvas()};$scope.alignGroupLeft=function(){if($scope.canvas.getActiveGroup===null)return;var arr=[];$scope.canvas.getActiveGroup().forEachObject(function(obj){arr.push(obj.get("left"))});var val=_.min(arr);$scope.canvas.getActiveGroup().forEachObject(function(obj){obj.set({"left":val});
obj.setCoords()});$scope.drawCanvas()};$scope.alignGroupTop=function(){if($scope.canvas.getActiveGroup===null)return;var arr=[];$scope.canvas.getActiveGroup().forEachObject(function(obj){arr.push(obj.get("top"))});var val=_.min(arr);$scope.canvas.getActiveGroup().forEachObject(function(obj){obj.set({"top":val});obj.setCoords()});$scope.drawCanvas()};$scope.alignGroupRight=function(){if($scope.canvas.getActiveGroup===null)return;var arr=[];$scope.canvas.getActiveGroup().forEachObject(function(obj){var left=
obj.get("left");var width=obj.get("scaleX")*obj.get("width");arr.push(left+width)});var val=_.max(arr);$scope.canvas.getActiveGroup().forEachObject(function(obj){var left=val-obj.get("scaleX")*obj.get("width");obj.set({"left":left});obj.setCoords()});$scope.drawCanvas()};$scope.alignGroupBottom=function(){if($scope.canvas.getActiveGroup===null)return;var arr=[];$scope.canvas.getActiveGroup().forEachObject(function(obj){var left=obj.get("top");var width=obj.get("scaleY")*obj.get("height");arr.push(left+
width)});var val=_.max(arr);$scope.canvas.getActiveGroup().forEachObject(function(obj){var top=val-obj.get("scaleY")*obj.get("height");obj.set({"top":top});obj.setCoords()});$scope.drawCanvas()};$scope.alignGroupVer=function(){if($scope.canvas.getActiveGroup===null)return;var arr=[];$scope.canvas.getActiveGroup().forEachObject(function(obj,index){var left=obj.get("left");var width=obj.get("scaleX")*obj.get("width")/2;arr.push(left+width)});var val=_.min(arr);$scope.canvas.getActiveGroup().forEachObject(function(obj){var left=
...
obj.get("height")/2;obj.set({"top":top});obj.setCoords()});$scope.drawCanvas()};$scope.deactiveGroup=function(){$scope.canvas.deactivateAll().renderAll();$scope.showAlignToolbar=false};$scope.unlockProportion=function(){if($scope.editableItem==null)return;$scope.canvas.setActiveObject($scope.canvas.item($scope.editableItem));$scope.drawCanvas()};$scope.rotateText=function(angle){if($scope.editableItem==null)return;if(!!angle){if(angle>360)angle=angle-360;if(angle<0)angle=angle+360;$scope.canvas.item($scope.editableItem).setAngle(angle);
$scope.drawCanvas()}else{$scope.canvas.item($scope.editableItem).setAngle(0);$scope.drawCanvas()}};$scope.deleteAllItem=function(){deleteAllObject();$scope.drawCanvas()};$scope.align=function(layout){if($scope.editableItem==null)return;$scope.setUndoRedo({element:$scope.canvas.item($scope.editableItem),parameters:JSON.stringify({"textAlign":$scope.canvas.item($scope.editableItem).textAlign}),interaction:"modify"});$scope.editable.textAlign=layout;$scope.canvas.item($scope.editableItem).setTextAlign(layout);
$scope.drawCanvas()};$scope.setOpacity=function(value){if($scope.editableItem==null)return;var value=1-value;$scope.canvas.item($scope.editableItem).set({opacity:value});$scope.drawCanvas()};$scope.setBackground=function(e){if($scope.editableItem==null)return;if(!e)$scope.canvas.item($scope.editableItem).setTextBackgroundColor("#"+$scope.background);else $scope.canvas.item($scope.editableItem).setTextBackgroundColor("");$scope.drawCanvas()};$scope.setLineHeight=function(){if($scope.editableItem==
null)return;$scope.canvas.item($scope.editableItem).setLineHeight($scope.lineHeight);$scope.drawCanvas()};$scope.changeCurve=function(val){if($scope.editableItem==null)return;var index=$scope.editableItem;var text=$scope.canvas.item(index).getText();if($scope.canvas.item(index).get("type")=="curvedText"){$scope.canvas.item(index).set({radius:(100-val)*2.5});$scope.drawCanvas();return}var curve=$scope.canvas.item(index);var option={};if($scope.rtlLang)option.rtl=true;option.left=curve.left;option.top=
curve.top;option.textAlign=curve.textAlign;option.fill=curve.fill;option.angle=curve.angle;option.textBackgroundColor=curve.textBackgroundColor;option.stroke=curve.stroke;option.fontSize=curve.fontSize;option.fontFamily=curve.fontFamily;option.lineHeight=curve.lineHeight;option.height=curve.height;option.width=curve.width;option.radius=(100-val)*2.5;var curvedtext=new fabric.CurvedText(text,option);$scope.canvas.remove($scope.canvas.getActiveObject());$scope.drawCanvas();$scope.canvas.add(curvedtext);
var index=$scope.canvas.getObjects().length-1;if(NBDESIGNCONFIG["lang_rtl"]=="rtl")$scope.setItemConfig($scope.canvas.item(index),"rtl",true);$scope.canvas.setActiveObject($scope.canvas.item(index));$scope.canvas.item($scope.editableItem).centerH();$scope.canvas.item($scope.editableItem).centerV();$scope.canvas.item($scope.editableItem).setCoords();$scope.drawCanvas()};$scope.setSpacing=function(){if($scope.editableItem==null)return;$scope.canvas.item($scope.editableItem).set("spacing",$scope.editable.spacing);
$scope.drawCanvas()};$scope.changeEffect=function(val){if($scope.editableItem==null)return;$scope.canvas.item($scope.editableItem).set("effect",val);$scope.drawCanvas()};$scope.reverseCurve=function(){if($scope.editableItem==null)return;var reverse=angular.element(document.querySelector("input#curve_reverse")).is(":checked");$scope.canvas.item($scope.editableItem).set("reverse",reverse);$scope.drawCanvas()};$scope.changePattern=function(path){var url=$scope.ajustUrl(path);changePattern(url);jQuery("#dg-pattern").modal("hide");
$scope.drawCanvas()};$scope.ajustUrl=function(path){return od_mainURL+path};$scope.loadPattern=function(){if($scope.patterns.length>0)return;PatternService.fn(function(data){if(data.flag=="1")$scope.patterns=data.data.patterns;else console.log("Error! Try again!")})};$scope.changePatternRepeat=function(val){$scope.canvas.item($scope.editableItem).fill.repeat=val;$scope.drawCanvas()};$scope.setFontSize=function(){if($scope.editableItem==null)return;$scope.canvas.item($scope.editableItem).setFontSize($scope.fontSize);
$scope.drawCanvas()};$scope.setGradient=function(){if($scope.editableItem==null)return;var option=createGradient();var direction="h";direction=angular.element(document.querySelector('input[name="gradient_direction"]:checked')).val();var width=$scope.canvas.item($scope.editableItem).getWidth();var height=$scope.canvas.item($scope.editableItem).getHeight();if(direction=="h")$scope.canvas.item($scope.editableItem).setGradient("fill",{type:"linear",x1:-width,y1:0,x2:0,y2:0,colorStops:option});else $scope.canvas.item($scope.editableItem).setGradient("fill",
{type:"linear",x1:0,y1:0,x2:0,y2:height/2,colorStops:option});$scope.drawCanvas()};$scope.zoomOut=function(){$scope.zoom=$scope.zoom-.1;if($scope.zoom<=1)$scope.zoom=1;$scope.canvas.setZoom($scope.zoom);$scope.drawCanvas();$scope.zoomCanvas()};$scope.zoomIn=function(){$scope.zoom=$scope.zoom+.1;if($scope.zoom>=2)$scope.zoom=2;$scope.zoomCanvas()};$scope.scaleItem=function(command){var obj=$scope.canvas.getActiveObject();if(!obj)return;var scaleX=obj.scaleX,scaleY=obj.scaleY,left=obj.left,top=obj.top,
width=obj.width*scaleX,height=obj.height*scaleY,factor=1;if(command=="+")factor=1.1;else factor=.9;var tempScaleX=scaleX*factor,tempScaleY=scaleY*factor,tempWidth=width*factor,tempHeight=height*factor,tempLeft=left+width/2-tempWidth/2,tempTop=top+height/2-tempHeight/2;obj.scaleX=tempScaleX;obj.scaleY=tempScaleY;obj.left=tempLeft;obj.top=tempTop;obj.setCoords();$scope.drawCanvas()};$scope.zoomCanvas=function(){var width=$scope.zoom*$scope.currentVariant.designArea["area_design_width"]*$scope.designScale;
var height=$scope.zoom*$scope.currentVariant.designArea["area_design_height"]*$scope.designScale;$scope.canvas.setDimensions({"width":width,"height":height});$scope.canvas.calcOffset();$scope.canvas.setZoom($scope.zoom);$scope.drawCanvas()};$scope.loadProduct=function(id,variation){ProductService.getProduct(id,variation).then(function(data){$scope.onLoadProduct=false;window.parent.NBDESIGNERPRODUCT.nbdesigner_ready();$scope.varaints.areaDesign=data.product;$scope.rateConvertCm2Px=Math.round($scope.rateConvertCm2Px*
(parseInt(data.dpi)/96)*10)/10;$scope.ajustmentScale = $scope.designScale;if(toType(data.design)=="object"){$scope.canvases=data.design;if(toType(data.ref)==="array")$scope.refProduct=data.ref;if(toType(data.config)==="object")$scope.ajustmentScale=data.config["scale"];else $scope.ajustmentScale=$scope.designScale;if(toType(data.fonts)==="array"){var font_url=window.parent.nbds_frontend.font_url;$scope.loadAdminDesignFont(data.fonts,font_url);return}$scope.changeOrientation();$scope.loaded=100;hide_indicator()}else{var sid="p_"+product_id,
cid="c_"+product_id;if(toType(localStorageService.get(sid))==="object"&&OD_task==""&&!$scope.ignoreLocalDesign){$scope.canvases=localStorageService.get(sid);if(toType(localStorageService.get(cid))==="object"){var config=localStorageService.get(cid);$scope.ajustmentScale=config["scale"];$scope.loadAdminDesignFont(config["font_used"],config["furl"])}}else if(toType(data.admindesign)==="object"){$scope.canvases=data.admindesign;if(toType(data.config)==="object")$scope.ajustmentScale=data.config["scale"];
if(toType(data.fonts)==="array"){var font_url=window.parent.nbds_frontend.font_url;$scope.loadAdminDesignFont(data.fonts,font_url)}}else{$scope.changeOrientation();$scope.loaded=100;hide_indicator()}}})};$scope.loadAdminListDesign=function(){if(!_.size($scope.adminListTemplate)){show_indicator();AdminTemplateService.getDesign(product_id).then(function(data){if(data["mes"]=="success")_.each(data.data,function(value,index){$scope.adminListTemplate.push({"src":value[0],"id":index})});else alert("Oops! Try again");
hide_indicator()})}$("#nbdesigner-list-template").addClass("open");$("#design-editor-container").removeClass("open")};$scope.loadExtraAdminDesign=function(id){$scope.loaded=0;$scope.ignoreLocalDesign=true;show_indicator();$("#dg-expand-feature").modal("hide");$scope.canvas.clear();$scope.loadProduct(id,0)};$scope.loadAdminDesignFont=function(fonts,_url){_.each(fonts,function(font){if(font.type=="google"){var name=font.name;name=name.replace(" ","+");jQuery("head").append("<link href='https://fonts.googleapis.com/css?family="+
...
null){$scope.editable._paths=[];$scope.editable.paths=[];return}var object=$scope.canvas.item($scope.editableItem);var type=object.get("type");if(type!="path-group"&&type!="path")return;$scope.editable._paths=object.paths;$scope.editable.paths=$scope.getColorPath(object.paths)};$scope.getColorPath=function(paths){var output=[];_.each(paths,function(path,key){if(toType(path.fill)=="string"&&path.fill.charAt(0)=="#"){var color=path.fill;color=color.slice(1,color.length);output.push({"fill":color,"key":key});
$scope.pathColor[key]=color}else if(toType(path.fill)=="string"&&path.fill.charAt(0)=="r"){var rgb=path.fill.slice(4,path.fill.length-1);var arr=rgb.split(",");var check=true;if(arr[0]=="0"&&arr[1]=="0"&&arr[2]=="0"&&!!arr[3])check=false;if(check){var R=parseInt(arr[0],16),G=parseInt(arr[1],16),B=parseInt(arr[2],16);var color=R+""+G+""+B;output.push({"fill":color,"key":key});$scope.pathColor[key]=color}}});return output};$scope.getColorCode=function(color){return"#"+color};$scope.updatePathColor=
function(color){if($scope.editable.type=="path-group")$scope.editable._paths[color.key].fill="#"+$scope.pathColor[color.key];$scope.drawCanvas()};$scope.showGrid=function(){};$scope.deleteLayer=function(e){deleteObject(e.index)};$scope.activeLayer=function(e){if($scope.canvas.item(e.index).get("selectable")&&$scope.canvas.item(e.index).get("visible")){$scope.canvas.setActiveObject($scope.canvas.item(e.index));$scope.drawCanvas()}else return};$scope.parseObject=function(str){var f=str.replace(/'/g,
'"');var e=JSON.parse(f);return e};$scope.changeVariant=function(){};$scope.changeOrientation=function(p){var first=false;if(p==null){p={"name":"frame_0","id":0};first=true}$scope.currentVariant.orientationBefore=$scope.currentVariant.orientationActive;if(!first)$scope.saveDesign($scope.currentVariant.orientationBefore);$scope.currentVariant.orientationActive=p.name;var i=0;$scope.currentVariant.info=[];_.each($scope.varaints.areaDesign,function(value,key){$scope.currentVariant.info[key]={"source":value,
"name":"frame_"+key,"v_index":$scope.currentVariant.index,"id":key};i++});var _img=[{"img_src":$scope.currentVariant.info[p.id]["source"]["img_src"],"img_src_left":$scope.currentVariant.info[p.id]["source"]["img_src_left"],"img_src_top":$scope.currentVariant.info[p.id]["source"]["img_src_top"],"img_src_width":$scope.currentVariant.info[p.id]["source"]["img_src_width"],"img_src_height":$scope.currentVariant.info[p.id]["source"]["img_src_height"]}];$scope.currentVariant.numberFrame=i;$scope.currentVariant.activeImages=
_img;$scope.currentVariant.designArea=$scope.varaints.areaDesign[p.id];var _width=$scope.currentVariant.designArea.area_design_width*$scope.designScale,_height=$scope.currentVariant.designArea.area_design_height*$scope.designScale;$scope.scale[p.name]=$scope.currentVariant.info[p.id]["source"]["real_width"]*$scope.rateConvertCm2Px/_width;$scope.currentScale=$scope.scale[p.name];$scope.canvas.setDimensions({"width":_width,"height":_height});if(!first)$scope.updateCurrentLayer();if(angular.isDefined($scope.canvases[p.name]))$scope.canvas.loadFromJSON($scope.canvases[p.name],
function(){applyImageFilters();$scope.canvas.renderAll();$scope.updateCurrentLayerAfterDelete();if(angular.isUndefined($scope.flagChangeOrientation[p.id])){$scope.resizeDesign($scope.designScale,$scope.ajustmentScale);$scope.fitReferenceDesign(p.id);$scope.flagChangeOrientation[p.id]=1}else if($scope.ignoreLocalDesign)$scope.resizeDesign($scope.designScale,$scope.ajustmentScale);else if($scope.ajustmentOrientationScale[p.id]!=$scope.designScale)$scope.resizeDesign($scope.designScale,$scope.ajustmentOrientationScale[p.id]);
$scope.ajustmentOrientationScale[p.id]=$scope.designScale});else $scope.canvas.clear();if(angular.isDefined($scope.fontCollection[p.name]))$scope.currentFontCollection=JSON.parse($scope.fontCollection[p.name]);$scope.updateUndoRedoStatus();$scope.showAlignToolbar=false;$scope.zoom=1;$scope.zoomCanvas();$scope.drawCanvas();$scope.canvas.isDrawingMode=false};$scope.resizeDesign=function(newScale,oldScale){if(newScale==oldScale)return;var factor=newScale/oldScale;$scope.canvas.forEachObject(function(obj){var scaleX=
obj.scaleX,scaleY=obj.scaleY,left=obj.left,top=obj.top,tempScaleX=scaleX*factor,tempScaleY=scaleY*factor,tempLeft=left*factor,tempTop=top*factor;obj.scaleX=tempScaleX;obj.scaleY=tempScaleY;obj.left=tempLeft;obj.top=tempTop;obj.setCoords()});$scope.canvas.calcOffset();$scope.drawCanvas()};$scope.snapGrid=function(){$scope.gridMode=!$scope.gridMode;jQuery(".canvas-container .upper-canvas").toggleClass("has-grid")};$scope.fitReferenceDesign=function(id){if(OD_ref=="")return;var ref_width=$scope.refProduct[id]["area_design_width"],
ref_height=$scope.refProduct[id]["area_design_height"],width=$scope.currentVariant.info[id]["source"]["area_design_width"],height=$scope.currentVariant.info[id]["source"]["area_design_height"];if(ref_width/ref_height>width/height){$scope.resizeDesign(width/ref_width,1);var offset_top=(height-ref_height)*$scope.zoom*$scope.designScale/2;$scope.canvas.forEachObject(function(obj){obj.top=obj.top+offset_top})}else{$scope.resizeDesign(height/ref_height,1);var offset_left=(width-ref_width)*$scope.zoom*
$scope.designScale/2;$scope.canvas.forEachObject(function(obj){obj.left=obj.left+offset_left})}$scope.canvas.calcOffset();$scope.drawCanvas()};$scope.saveDesign=function(orientation){if(orientation==null)orientation=$scope.currentVariant.orientationActive;$scope.zoom=1;$scope.zoomCanvas();$scope.drawCanvas();var json=canvas.toJSON(["itemId","selectable","lockMovementX","lockMovementY","lockScalingX","lockScalingY","lockRotation","rtl","elementUpload","forceLock"]);if($scope.modeMobile){var _width=
$scope.currentVariant.designArea.area_design_width*$scope.designScale,_height=$scope.currentVariant.designArea.area_design_height*$scope.designScale;if(_width>_height)if(_width*$scope.currentScale>1E3)$scope.zoom=1E3/_width;else $scope.zoom=$scope.currentScale;else if(_height*$scope.currentScale>1E3)$scope.zoom=1E3/_height;else $scope.zoom=$scope.currentScale}else $scope.zoom=$scope.currentScale;$scope.zoomCanvas();$scope.drawCanvas();var svg=canvas.deactivateAll().toDataURL();$scope.canvases[orientation]=
json;$scope.fontCollection[orientation]=JSON.stringify($scope.currentFontCollection);$scope.dataCustomerDesign[orientation]=svg;$scope.currentPreview=svg;$scope.zoom=1;$scope.zoomCanvas();$scope.drawCanvas();var sid="p_"+product_id,cid="c_"+product_id;config={};config["scale"]=$scope.designScale;config["furl"]=window.parent.nbds_frontend.font_url;config["font_used"]=$scope.filterFontUsed();try{localStorageService.set(sid,$scope.canvases);localStorageService.set(cid,config)}catch(e){localStorageService.clearAll();
localStorageService.set(sid,$scope.canvases);localStorageService.set(cid,config)}};$scope.usedFont=function(cv){var font=[];cv.forEachObject(function(object){if(object.type==="i-text"||object.type==="text"||object.type==="curvedText")font.push(object.get("fontName"))});return font};$scope.filterFontUsed=function(){var usedFonts=[];_.forEach($scope.fontCollection,function(value){var fonts=JSON.parse(value);_.each(fonts,function(v){if(toType(v)=="object")if(v.cat[0]=="99")usedFonts.push({"type":"google",
"name":v.name});else usedFonts.push({"type":"custom","name":v.alias,"ext":v.type})})});return usedFonts};$scope.preview=function(){$scope.saveDesign()};$scope.storeDesign=function(){$scope.saveDesign();var fonts=$scope.filterFontUsed(),config={};config["scale"]=$scope.designScale;NBDESIGNERCART.upload_design($scope.dataCustomerDesign,$scope.canvases,fonts,config)};$scope.storeDesign1=function(){show_indicator();localStorageService.set("design",$scope.canvases);$scope.currentUser="";var fonts={},images=
{};$scope.saveDesign();$scope.zoom=5;$scope.zoomCanvas();var orientationActive=$scope.currentVariant.orientationActive;_.each($scope.canvases,function(value,key){var key="frame_1";var p={"name":key};$scope.changeOrientation(p);fonts[key]=$scope.usedFont($scope.canvas);if(/MSIE/.test(navigator.userAgent))images[key]=$scope.canvas.deactivateAll().toSVG();else images[key]=$scope.canvas.deactivateAll().toSVG()});$scope.changeOrientation({"name":orientationActive});console.log($scope.canvas.deactivateAll().toSVG());
UserService.getUserId().then(function(data){$scope.currentUser=data;if($scope.currentUser!==""||$scope.currentUser>0)if(/MSIE/.test(navigator.userAgent))var query="type=saveDesignIE";else var query="type=saveDesign";if($scope.currentUser!=0&&$scope.currentUser!="")StoreService.postData(fonts,images,query);$scope.zoom=1;$scope.zoomCanvas();hide_indicator()})};$scope.toggleImportArea=function(){$("#design-editor-container").toggleClass("open");$("#nbdesigner-list-template").removeClass("open");$("#nbdesigner-list-template").stop().animate({scrollTop:0},
400)};$scope.exportDesign=function(){$scope.saveDesign();var filename="design.json",text=JSON.stringify($scope.canvases),a=document.createElement("a");a.setAttribute("href","data:text/plain;charset=utf-u,"+text);a.setAttribute("download",filename);a.click()};$scope.downloadPNG=function(){$scope.saveDesign();_.each($scope.dataCustomerDesign,function(val,key){var filename=key+".png",a=document.createElement("a");a.setAttribute("href",val);a.setAttribute("download",filename);a.click()})};$scope.downloadJPG=
function(){$scope.saveDesign();_.each($scope.dataCustomerDesign,function(val,key){var filename=key+".jpg",a=document.createElement("a");a.setAttribute("href",val);a.setAttribute("download",filename);a.click()})};$scope.importDesign=function(){var datadesign=JSON.parse($("#design-json-content").val());if(toType(datadesign)==="object"){show_indicator();$scope.canvases=datadesign;$scope.changeOrientation();hide_indicator()}$("#dg-expand-feature").modal("hide")};$scope.resetAdminDesign=function(){$scope.loaded=
0;$scope.ignoreLocalDesign=true;show_indicator();$("#dg-expand-feature").modal("hide");$scope.canvas.clear();$scope.loadProduct("",0);$scope.clearHistory()};$scope.calcLeft=function(left){if(angular.isDefined(left)){var offsetWidth=$scope.designerWidth/2;left*=$scope.designScale;return offsetWidth-(offsetWidth-left)*$scope.zoom}};$scope.calcWidthThumb=function(n){if(n<4)return n;else return 4};$scope.calcDimension=function(width){width=$scope.zoom*width*$scope.designScale;width=width+"px";return width};
...
if(type==="path-group")layer={"name":"SVG","type":"path-group","src":od_mainURL+"images/svg.png","index":index};if(type==="group")layer={"name":langjs["LAYER_GROUP"],"type":"path-group","src":od_mainURL+"images/svg.png","index":index};if(type==="path")layer={"name":langjs["LAYER_PATH"],"type":"path","src":od_mainURL+"images/path.png","index":index};if(type==="rect")layer={"name":langjs["LAYER_RECTANGLE"],"type":"rect","src":od_mainURL+"images/mark/mark_12.svg","index":index};if(type==="circle")layer=
{"name":langjs["LAYER_CIRCLE"],"type":"circle","src":od_mainURL+"images/mark/mark_03.svg","index":index};if(type==="triangle")layer={"name":langjs["LAYER_TRIANGLE"],"type":"triangle","src":od_mainURL+"images/mark/mark_13.svg","index":index};if(type==="line")layer={"name":langjs["LAYER_LINE"],"type":"line","src":od_mainURL+"images/path.png","index":index};if(type==="polygon")layer={"name":langjs["LAYER_POLYGON"],"type":"polygon","src":od_mainURL+"images/mark/mark_11.svg","index":index};if(!object.get("selectable")||
...
$scope.texturePatternBrush;else $scope.canvas.freeDrawingBrush=new fabric[type+"Brush"]($scope.canvas);jQuery("#brush_width").slider("value",0);jQuery("#brush_shadow_width").slider("value",0);if(!jQuery("#show_popover_option_draw").hasClass("open"))jQuery("#show_popover_option_draw").triggerHandler("click")};$scope.setDrawingLineColor=function(value){value="#"+value;if($scope.canvas.freeDrawingBrush)$scope.canvas.freeDrawingBrush.color=value};$scope.setDrawingLineWidth=function(value){if($scope.canvas.freeDrawingBrush)$scope.canvas.freeDrawingBrush.width=
...
$scope.initHLinePatternBrush();$scope.initSquarePatternBrush();$scope.initDiamondPatternBrush();$scope.initImagePatternBrush()};$scope.initImagePatternBrush=function(){var img=new Image;img.src=od_mainURL+"images/honey_im_subtle.png";$scope.texturePatternBrush=new fabric.PatternBrush(canvas);$scope.texturePatternBrush.source=img};$scope.initDiamondPatternBrush=function(){$scope.diamondPatternBrush=new fabric.PatternBrush(canvas);$scope.diamondPatternBrush.getPatternSrc=function(){var squareWidth=
...
{x:60,y:5.359},{x:40,y:20},{x:20,y:5.359},{x:22.679,y:30},{x:0,y:40},{x:22.679,y:50},{x:20,y:74.641}],{left:0,top:0,fill:"#"+$scope.getRandomColor()}));$scope.ajustAfterAddItem("polygon")};$scope.setShapeColor=function(e){if($scope.editable.type==="line")$scope.changeStroke(e);else $scope.changeColor(e)};$scope.dosomething=function(){console.log("hello kitty")};$scope.onFileSelect=function($files){$scope.errorUpload="";$scope.progressUpload=0;$scope.file=[];var max_size=parseInt(NBDESIGNCONFIG["max_size_upload"]);
...
function(e){$timeout(function(){$scope.src=e.target.result})}}(fileReader,i)}};$scope.startUpload=function(){if($scope.errorUpload==""){jQuery('#upload-tabs a[href="#uploaded-photo"]').tab("show");$scope.loading=true;for(var i=0;i<$scope.file.length;i++){$scope.progressUpload=1;$scope.upload=$upload.upload({url:window.parent.nbds_frontend.url,method:"POST",withCredentials:true,data:{"action":"nbdesigner_customer_upload","nonce":window.parent.nbds_frontend.nonce},file:$scope.file[i]}).progress(function(evt){$scope.progressUpload=
parseInt(100*evt.loaded/evt.total)}).success(function(data,status,headers,config){if(angular.isUndefined(data.src))alert("The file exist or upload failed, try later !!");else{$scope.uploadURL.push(data.src);localStorageService.set($scope.iid,$scope.uploadURL)}$scope.loading=false})}if($scope.file.length==0)$scope.loading=false}else alert($scope.errorUpload)};$scope.loadLocalStorageImage=function(){$scope.disableDrawMode();var localSource=localStorageService.get($scope.iid,$scope.uploadURL);if(!!localSource&&
toType(localSource)=="array")$scope.uploadURL=localSource;$scope.loaded=30};$scope.hasGetUserMedia=function(){return!!(navigator.getUserMedia||navigator.webkitGetUserMedia||navigator.mozGetUserMedia||navigator.msGetUserMedia)};$scope.initWebcam=function(){$scope.statusWebcam=true;Webcam.set({width:400,height:300,dest_width:1280,dest_height:960,image_format:"jpeg",jpeg_quality:100});Webcam.attach("#my_camera");Webcam.setSWFLocation(window.parent.nbds_frontend.url_style+"webcam.swf")};$scope.takeSnapshot=
function(){show_indicator();Webcam.snap(function(data_uri){$scope.resetWebcam();$scope.statusWebcam=false;var raw_image_data=data_uri.replace(/^data\:image\/\w+\;base64\,/,"");jQuery.ajax({url:window.parent.nbds_frontend.url,method:"POST",data:{"action":"nbdesigner_save_webcam_image","image":raw_image_data,"nonce":window.parent.nbds_frontend.nonce}}).done(function(data){hide_indicator();data=JSON.parse(data);if(data.flag=="success"){jQuery("#dg-myclipart").modal("hide");$scope.addImage(data.url)}else{alert("Oops! Try again!");
...
"")return;nbdesigner_fb1()};$scope.addImage=function(url,params){$scope.zoom=1;$scope.zoomCanvas();fabric.Image.fromURL(url,function(op){jQuery("#dg-myclipart").modal("hide");jQuery("#dg-qrcode").modal("hide");if(params){var element=$scope.canvas.getActiveObject().getElement();$scope.setUndoRedo({element:$scope.canvas.getActiveObject(),parameters:JSON.stringify({"src":element.src}),interaction:"modify"});element.setAttribute("src",url);$scope.updateCurrentLayerAfterDelete()}else{$scope.canvas.add(op);
$scope.showPopover("art");$scope.canvas.setActiveObject($scope.canvas.item($scope.canvas.getObjects().length-1));var max_width=$scope.canvas.width*.9;var max_height=$scope.canvas.height*.9;var new_width=max_width;if(op.width<max_width)new_width=op.width;var width_ratio=new_width/op.width;var new_height=op.height*width_ratio;if(new_height>max_height){new_height=max_height;var height_ratio=new_height/op.height;new_width=op.width*height_ratio}$scope.canvas.getActiveObject().set({scaleX:new_width/op.width,
scaleY:new_height/op.height});$scope.ajustAfterAddItem("image")}$scope.drawCanvas()},{crossOrigin:"anonymous"})};$scope.addImageFromUrl=function(){if($scope.imageFromUrl==""){alert("Image url empty!");return}show_indicator();fabric.util.loadImage($scope.imageFromUrl,function(img){if(img==null)alert("Error!");else jQuery.ajax({url:window.parent.nbds_frontend.url,method:"POST",data:{"action":"nbdesigner_copy_image_from_url","nonce":window.parent.nbds_frontend.nonce,"url":$scope.imageFromUrl}}).done(function(data){data=
JSON.parse(data);if(data["flag"]==1){hide_indicator();$scope.addImage(data["src"])}else{hide_indicator();jQuery('#upload-tabs a[href="#upload-computer"]').tab("show");alert("Try to download image and then upload to our server!")}})})};$scope.resetImage=function(){var object=$scope.canvas.getActiveObject();if(object==null)return;object.filters=[];object.applyFilters();var _url="";if($scope.canvas.item($scope.editableItem).get("type")=="custom-image"){_url=$scope.canvas.item($scope.editableItem).origin_src;
if(_url!=""){$scope.canvas.remove($scope.canvas.getActiveObject());$scope.addImage(_url)}}$scope.drawCanvas()};$scope.loadSource=function(){var localSource=localStorageService.get($scope.iid,$scope.uploadURL);if(!!localSource&&toType(localSource)=="array")$scope.uploadURL=localSource;$scope.loaded=30};$scope.initCropCanvas=function(type){canvas2.clear();canvas2.calcOffset();canvas2.setZoom(1);canvas2.renderAll();canvas2.isDrawingMode=false;if($scope.editableItem==null)return;if(type==="crop"){$scope.cropToolMode=
true;$scope.clipPathMode=false}else{$scope.cropToolMode=false;$scope.clipPathMode=true}var width=400,height=400;var _url="";if($scope.canvas.item($scope.editableItem).get("type")=="image")_url=$scope.canvas.item($scope.editableItem).getSvgSrc();if($scope.canvas.item($scope.editableItem).get("type")=="custom-image")_url=$scope.canvas.item($scope.editableItem).origin_src;var url=_url,_type=$scope.canvas.item($scope.editableItem).type;if(url=="")return;if(!$scope.modeMobile){width=600;height=400}else{width=
$(window).width()-20;height=250}canvas2.setDimensions({"width":width,"height":height});$("#shape_canvas").css({"width":width,"height":height});var ratio=1;var originImg=new Image;originImg.onload=function(){op=new fabric.Image(originImg);canvas2.add(op);var new_width=op.width;if(op.width>width)new_width=width;var width_ratio=op.width/new_width;var new_height=op.height/width_ratio;ratio=width_ratio;if(new_height>400){new_height=400;var height_ratio=op.height/new_height;new_width=op.width/height_ratio;
ratio=height_ratio}canvas2.item(0).set({top:0,left:0,scaleX:1/ratio,scaleY:1/ratio,selectable:false,hasControls:false,hasBorders:false,opacity:.7}).center().setCoords();canvas2.renderAll();if($scope.cropToolMode){var path=new fabric.Path(listPath.path_crop),path2=new fabric.Path(listPath.path_crop);$scope.currentPathCrop=listPath.path_crop;path.set({originX:"center",originY:"center",scaleX:ratio,scaleY:ratio,fill:"#9bf0e9",opacity:0});path2.set({originX:"center",originY:"center",opacity:0,hasRotatingPoint:false,
centeredScaling:true})}else{var path=new fabric.Path(listPath.path_1),path2=new fabric.Path(listPath.path_1);$scope.currentPathCrop=listPath.path_1;path.set({originX:"center",originY:"center",scaleX:ratio*3,scaleY:ratio*3,fill:"#9bf0e9",opacity:0});path2.set({originX:"center",originY:"center",opacity:0,scaleX:3,scaleY:3,hasRotatingPoint:false,centeredScaling:true})}canvas2.renderAll();var op2=new fabric.Image(originImg,{top:0,left:0,scaleX:1/ratio,scaleY:1/ratio,selectable:false,hasControls:false,
hasBorders:false});canvas2.add(op2);var c=document.getElementById("shape_canvas");var _ctx=c.getContext("2d");var shape=canvas.item(1);canvas2.item(1).set({clipTo:function(ctx){path.render(ctx)}}).center().setCoords();canvas2.add(path2);canvas2.item(2).set({borderColor:"rgba(220, 220, 220, 1)"});canvas2.item(2)["setControlVisible"]("tl",false);canvas2.item(2)["setControlVisible"]("bl",false);canvas2.item(2).center().setCoords();canvas2.setActiveObject(canvas2.item(2));canvas2.renderAll()};originImg.src=
url};$scope.cropImage=function(){var _url="";if($scope.canvas.item($scope.editableItem).get("type")=="image")_url=$scope.canvas.item($scope.editableItem).getSvgSrc();if($scope.canvas.item($scope.editableItem).get("type")=="custom-image")_url=$scope.canvas.item($scope.editableItem).get("origin_src");var url=_url,width=canvas2.item(2).get("width"),height=canvas2.item(2).get("height"),scaleX=canvas2.item(2).get("scaleX"),left=canvas2.item(2).get("left"),top=canvas2.item(2).get("top"),i_top=canvas2.item(1).get("top"),
i_left=canvas2.item(1).get("left"),ratio=1/canvas2.item(0).get("scaleX");canvas2.remove(canvas2.item(0));canvas2.remove(canvas2.item(2));var length=canvas2.getObjects().length;while(length>1){canvas2.remove(canvas2.item(1));length=canvas2.getObjects().length}canvas2.item(0).set({"left":scaleX*width/2-left+i_left,"top":scaleX*height/2-top+i_top});canvas2.setDimensions({"width":scaleX*width*ratio,"height":scaleX*height*ratio});canvas2.calcOffset();canvas2.setZoom(ratio);canvas2.renderAll();var img=
new Image;img.src=canvas2.deactivateAll().toDataURL();img.onload=function(){var f_img=new fabric.CustomImage(img,{"origin_src":url});$scope.canvas.remove($scope.canvas.getActiveObject());$scope.canvas.add(f_img);var op_width=scaleX*width*ratio,op_height=scaleX*height*ratio;jQuery("#dg-crop-image").modal("hide");var max_width=$scope.canvas.width*.7;var max_height=$scope.canvas.height*.7;var new_width=max_width;if(op_width<max_width)new_width=op_width;var width_ratio=new_width/op_width;var new_height=
op_height*width_ratio;if(new_height>max_height){new_height=max_height;var height_ratio=new_height/op_height;new_width=op_width*height_ratio}var index=$scope.canvas.getObjects().length-1;$scope.canvas.setActiveObject($scope.canvas.item(index));$scope.addLayers($scope.canvas.getActiveObject(),"custom-image",index);$scope.canvas.getActiveObject().set({scaleX:new_width/op_width,scaleY:new_height/op_height}).center().setCoords();$scope.drawCanvas()}};$scope.lassoTool=function(){$scope.cropToolMode=true;
$scope.clipPathMode=false;canvas2.remove(canvas2.item(2));canvas2.item(1).set({clipTo:function(ctx){ctx.rect(0,0,0,0)}});canvas2.renderAll();canvas2.isDrawingMode=true;$scope.freeDrawingMode="Pencil";canvas2.freeDrawingBrush=new fabric["PencilBrush"](canvas2)};$scope.cropTool=function(){$scope.cropToolMode=true;$scope.clipPathMode=false;$scope.currentPathCrop=listPath.path_crop;path2=new fabric.Path($scope.currentPathCrop);path2.set({originX:"center",originY:"center",opacity:0,hasRotatingPoint:false,
centeredScaling:true});canvas2.remove(canvas2.item(2));canvas2.add(path2);canvas2.item(2).center().setCoords();canvas2.item(2)["setControlVisible"]("tl",false);canvas2.item(2)["setControlVisible"]("bl",false);canvas2.setActiveObject(canvas2.item(2));canvas2.renderAll()};$scope.clipPath=function(type){var index="path_"+type;$scope.currentPathCrop=listPath[index];path2=new fabric.Path($scope.currentPathCrop);path2.set({originX:"center",originY:"center",opacity:0,hasRotatingPoint:false,scaleX:3,scaleY:3,
centeredScaling:true});canvas2.remove(canvas2.item(2));canvas2.add(path2);canvas2.item(2).center().setCoords();canvas2.item(2)["setControlVisible"]("tl",false);canvas2.item(2)["setControlVisible"]("bl",false);canvas2.setActiveObject(canvas2.item(2));canvas2.renderAll()};$scope.toggleLockLayer=function(layer){var bool=!$scope.canvas.item(layer.index).get("selectable"),id="#layer-"+layer.index,lock=$scope.canvas.item(layer.index).get("forceLock");if($scope.task!=="admindesign"){if(lock)return}else $scope.canvas.item(layer.index).set({"forceLock":!lock});
$scope.canvas.item(layer.index).set({selectable:bool});if(bool){jQuery(id).removeClass("lock");jQuery(id+" .nbdesigner_lock_layer i").removeClass("fa-lock");jQuery(id+" .nbdesigner_lock_layer i").addClass("fa-unlock-alt")}else{jQuery(id).addClass("lock");jQuery(id+" .nbdesigner_lock_layer i").addClass("fa-lock");jQuery(id+" .nbdesigner_lock_layer i").removeClass("fa-unlock-alt")}$scope.canvas.deactivateAll();$scope.drawCanvas()};$scope.lockItem=function(command){var object=$scope.canvas.getActiveObject();
if(object==null)return;switch(command){case "a":var index=$scope.canvas.getObjects().indexOf($scope.canvas.getActiveObject());$scope.toggleLockLayer({"index":index});break;case "x":var val=!object.get("lockMovementX");object.set({lockMovementX:val});break;case "y":var val=!object.get("lockMovementY");object.set({lockMovementY:val});break;case "sx":var val=!object.get("lockScalingX");object.set({lockScalingX:val});break;case "sy":var val=!object.get("lockScalingY");object.set({lockScalingY:val});break;
case "r":var val=!object.get("lockRotation");object.set({lockRotation:val});break;default:break}$scope.getStatusItem()};$scope.getStatusItem=function(){var object=$scope.canvas.getActiveObject();if(object==null)return;$scope.editable.lockMovementX=object.get("lockMovementX");$scope.editable.lockMovementY=object.get("lockMovementY");$scope.editable.lockScalingX=object.get("lockScalingX");$scope.editable.lockScalingY=object.get("lockScalingY");$scope.editable.lockRotation=object.get("lockRotation");
$scope.editable.selectable=object.get("selectable")};$scope.toggleVisibleLayer=function(layer){var bool=!$scope.canvas.item(layer.index).get("visible"),id="#layer-"+layer.index;$scope.canvas.item(layer.index).set({visible:bool});if(bool){jQuery(id).removeClass("lock");jQuery(id).addClass("active");jQuery(id+" .nbdesigner_visible_layer i").addClass("fa-eye");jQuery(id+" .nbdesigner_visible_layer i").removeClass("fa-eye-slash")}else{jQuery(id).addClass("lock");jQuery(id).removeClass("active");jQuery(id+
" .nbdesigner_visible_layer i").removeClass("fa-eye");jQuery(id+" .nbdesigner_visible_layer i").addClass("fa-eye-slash")}$scope.canvas.deactivateAll();$scope.drawCanvas()};$scope.duplicateItem=function(){var object=canvas.getActiveObject();if(object==null)return;var obj=fabric.util.object.clone(object);$scope.canvas.add(obj);var index=$scope.canvas.getObjects().length-1;$scope.canvas.deactivateAll();$scope.canvas.setActiveObject($scope.canvas.item(index));$scope.drawCanvas();$scope.updateCurrentLayerAfterDelete()};
$scope.setUndoRedo=function(undo,redo){if(undo){if(angular.isUndefined($scope.undos[$scope.currentVariant.orientationActive]))$scope.undos[$scope.currentVariant.orientationActive]=[];$scope.undos[$scope.currentVariant.orientationActive].push(undo);if($scope.undos[$scope.currentVariant.orientationActive].length>20)$scope.undos[$scope.currentVariant.orientationActive].shift()}if(redo){if(angular.isUndefined($scope.redos[$scope.currentVariant.orientationActive]))$scope.redos[$scope.currentVariant.orientationActive]=
...
var parameters=JSON.parse(_parameters);_.each(parameters,function(val,key){parameters[key]=item.get(key)});_parameters=JSON.stringify(parameters);$scope.setItemParameters(last.parameters,last.element.itemId)}$scope.setUndoRedo({element:last.element,parameters:_parameters,interaction:last.interaction});$scope.drawCanvas()}};$scope.getItemJson=function(item,params){if(params)return JSON.stringify(params);else return JSON.stringify(item)};$scope.setItemParameters=function(parameters,itemId){var params=
JSON.parse(parameters);var item=$scope.canvas.item($scope.getItemById(itemId));if(typeof params.src!=="undefined")fabric.Image.fromURL(params.src,function(op){item.getElement().setAttribute("src",params.src);$scope.updateCurrentLayerAfterDelete();setTimeout(function(){$scope.drawCanvas()},1)});item.set(params);item.setCoords()};$scope.updateUndoRedoStatus=function(){$scope.orientationActiveUndoStatus=false;$scope.orientationActiveRedoStatus=false;if(angular.isDefined($scope.undos[$scope.currentVariant.orientationActive]))if($scope.undos[$scope.currentVariant.orientationActive].length>
0)$scope.orientationActiveUndoStatus=true;if(angular.isDefined($scope.redos[$scope.currentVariant.orientationActive]))if($scope.redos[$scope.currentVariant.orientationActive].length>0)$scope.orientationActiveRedoStatus=true};$scope.getItemById=function(id){var len=$scope.canvas.getObjects().length;for(var i=0;i<len;i++)if($scope.canvas.item(i).get("itemId")==id)return i};$scope.setElementUpload=function(){if($scope.editableItem==null)return;var object=$scope.canvas.item($scope.editableItem);if(object.type===
"image"||object.type==="custom-image"){if(typeof object.elementUpload==="undefined")object.set({"elementUpload":true});else{var hasUpload=!object.elementUpload;object.set({"elementUpload":hasUpload})}$scope.updatePositionReplaceButton()}return};$scope.updatePositionReplaceButton=function(){var object=$scope.canvas.item($scope.editableItem);$("#replace-element-upload").hide();if(object.type==="image"||object.type==="custom-image")if(typeof object.elementUpload!=="undefined")if(object.elementUpload){var left=
object.left+10,top=object.top+10;if(left<10)left=10;if(top<10)top=10;left=left+$scope.calcLeft($scope.currentVariant.designArea["area_design_left"]);top=top+$scope.calcLeft($scope.currentVariant.designArea["area_design_top"]);$("#replace-element-upload").css({left:left+"px",top:top+"px"}).show()}return};$scope.preReplaceImage=function(){$scope.readyReplaceImage=true;$("#dg-myclipart").modal("show")};$scope.init=function(){show_indicator();if(!checkNavigator()){hide_indicator();return}$scope.canvas=
canvas;$scope.loaded=0;$scope.loadSource();$scope.loadLanguage();if(OD_temp!="")$scope.loadProduct(OD_temp,0);else $scope.loadProduct("");$scope.initBrushes();window.parent.NBDESIGNERPRODUCT.nbdesigner_ready();if(checkFireFox()||checkIE())jQuery("input.jscolor").removeAttr("disabled")};$scope.init();$scope.debug=function(){};$scope.changeBackgroundId=function(n){$scope.backgroundId=n};$scope.state="pub";window.scope=$scope});
...
angular.module("app").service("ProductService",function($http,$q){this.getProduct=function(id,variation){var deferred=$q.defer(),data={action:"nbdesigner_get_product_info",nonce:window.parent.nbds_frontend.nonce,id:product_id};$http({url:window.parent.nbds_frontend.url,method:"POST",data:"action=nbdesigner_get_product_info&nonce="+window.parent.nbds_frontend.nonce+"&id="+product_id+"&oid="+orderid+"&did="+id+"&variation="+variation+"&ref="+OD_ref+"&folder="+OD_folder,headers:{"Content-Type":"application/x-www-form-urlencoded"}}).success(function(data,
...
var NBDESIGNERCART={dataImage:{},dataJson:{},flag:0,setDataImage:function(data){this.dataImage=data},setDataJson:function(data){this.dataJson=data},setFlag:function(){this.flag=0},upload_design:function(data_img,data_json,fonts,config){show_indicator();data_json=JSON.stringify(data_json);_fonts=JSON.stringify(fonts);_config=JSON.stringify(config);var data={"img":data_img,"json":data_json};jQuery.ajax({url:window.parent.nbds_frontend.url,method:"POST",data:{"action":"nbdesigner_save_customer_design",
"image":data,"nonce":window.parent.nbds_frontend.nonce,"sid":window.parent.nbds_frontend.sid,"adid":adid,"product_id":product_id,"orderid":orderid,"task":OD_task,"fonts":_fonts,"config":_config,"priority":OD_priority,"save_status":OD_save_status,"oiid":OD_oiid,"folder":OD_folder}}).done(function(data){var _data=JSON.parse(data);if(_data.flag=="success"){window.parent.NBDESIGNERPRODUCT.show_design_thumbnail(_data.image);window.parent.hideDesignFrame();if(OD_task==""&&orderid=="")window.parent.NBDESIGNERPRODUCT.get_sugget_design(product_id);
if(_data.redesign!="")window.parent.hideDesignFrame(_data.redesign);else window.parent.hideDesignFrame()}else if(_data.flag=="pendding"){if(_data.redesign!="")window.parent.hideDesignFrame(_data.redesign)}else alert("Something wrong! Please try again...");OD_save_status=1;hide_indicator()})},ajax_upload:function(data_img,data_json,flag){var key="frame_"+flag;var dataimg={};dataimg[key]=data_img[key];console.log(dataimg);var data={"img":dataimg,"json":data_json};self=this;jQuery.ajax({url:window.parent.nbds_frontend.url,
method:"POST",data:{"action":"nbdesigner_save_customer_design","image":data,"nonce":window.parent.nbds_frontend.nonce,"sid":window.parent.nbds_frontend.sid,"product_id":product_id,"orderid":orderid}}).done(function(data){var _data=JSON.parse(data);if(_data.flag=="success"){var next_flag=parseInt(self.flag)+1;self.flag=next_flag;var next_key="frame_"+self.flag;if(self.dataImage[next_key]){console.log(self.flag);self.ajax_upload(self.dataImage,self.dataJson,self.flag)}else{console.log("complete!");
hide_indicator()}}else if(_data.flag=="pendding"){if(_data.redesign!="")window.parent.hideDesignFrame(_data.redesign)}else alert("Something wrong! Please try again...");hide_indicator()})}};
...
value){html+='<img class="img-responsive img-thumbnail nbdesigner_upload_image shadow hover-shadow" data-url="" onclick="NBDESIGNERFUNC.addFacebookImage(\''+value+'\', this)" src="'+value+'" />'});$("#uploaded-facebook").append(html)}}})}else if(response.status==="not_authorized");else;})};
...
var NBDESIGNERFUNC={addFacebookImage:function(url,e){var _url=jQuery(e).attr("data-url");var sefl=jQuery(e);var scope=angular.element(document.getElementById("designer-controller")).scope();if(_url!="")scope.addImage(_url);else{show_indicator();jQuery.ajax({url:window.parent.nbds_frontend.url,method:"POST",data:{"action":"nbdesigner_copy_image_from_url","nonce":window.parent.nbds_frontend.nonce,"url":url}}).done(function(data){data=JSON.parse(data);if(data["flag"]==1){sefl.attr("data-url",data["src"]);
scope.addImage(data["src"])}else{alert("Try to download image and then upload to our server!");jQuery('#upload-tabs a[href="#upload-computer"]').tab("show")}hide_indicator()})}if(scope.$root.$$phase!=="$apply"&&scope.$root.$$phase!=="$digest")scope.$apply()},addQrImage:function(url){var scope=angular.element(document.getElementById("designer-controller")).scope();scope.addImage(url);if(scope.$root.$$phase!=="$apply"&&scope.$root.$$phase!=="$digest")scope.$apply()}};
...
jQuery(document).ready(function($){window.parent.jQuery(".variations_form.cart").on("change",function(){var variation_id=window.parent.jQuery('input[name="variation_id"]').val();var scope=angular.element(document.getElementById("designer-controller")).scope();if(variation_id!="")window.parent.NBDESIGNERPRODUCT.nbdesigner_unready();if(!scope.onLoadProduct&&variation_id!=""){scope.loaded=0;scope.onLoadProduct=true;show_indicator();scope.canvas.clear();scope.loadProduct(product_id,variation_id)}if(scope.$root.$$phase!==
"$apply"&&scope.$root.$$phase!=="$digest")scope.$apply()});window.parent.jQuery(".reset_variations").on("click",function(){var scope=angular.element(document.getElementById("designer-controller")).scope();window.parent.NBDESIGNERPRODUCT.nbdesigner_unready();if(!scope.onLoadProduct){scope.loaded=0;scope.onLoadProduct=true;show_indicator();scope.canvas.clear();scope.loadProduct(product_id,0)}if(scope.$root.$$phase!=="$apply"&&scope.$root.$$phase!=="$digest")scope.$apply()});$(document).bind("keydown",
function(e){var scope=angular.element(document.getElementById("designer-controller")).scope();var object=scope.canvas.getActiveObject();var $target=$(e.target);if(e.shiftKey)scope.holdShiftKey=true;else scope.holdShiftKey=false;switch(e.which){case 37:if(object==null||$target.is('textarea,input[type="text"],input[type="number"]'))return;if(!object.get("lockMovementX"))scope.ShiftLeft();break;case 39:if(object==null||$target.is('textarea,input[type="text"],input[type="number"]'))return;if(!object.get("lockMovementX"))scope.ShiftRight();
break;case 38:if(object==null||$target.is('textarea,input[type="text"],input[type="number"]'))return;if(!object.get("lockMovementY"))scope.ShiftUp();break;case 40:if(object==null||$target.is('textarea,input[type="text"],input[type="number"]'))return;if(!object.get("lockMovementY"))scope.ShiftDown();break;case 46:if(object==null||$target.is('textarea,input[type="text"],input[type="number"]'))return;deleteObject(scope.editableItem);break;case 67:if(object==null||$target.is('textarea,input[type="text"],input[type="number"]'))return;
if(e.ctrlKey)scope.duplicateItem();break;case 90:if(e.ctrlKey)scope.undoDesign();break;case 89:if(e.ctrlKey)scope.redoDesign();break;case 107:if(object==null||$target.is('textarea,input[type="text"],input[type="number"]'))return;if(e.shiftKey)scope.scaleItem("+");break;case 109:if(object==null||$target.is('textarea,input[type="text"],input[type="number"]'))return;if(e.shiftKey)scope.scaleItem("-");break;default:return}if(scope.$root.$$phase!=="$apply"&&scope.$root.$$phase!=="$digest")scope.$apply()});
$("#dg-myclipart").on("hidden.bs.modal",function(){var scope=angular.element(document.getElementById("designer-controller")).scope();scope.readyReplaceImage=false})});initTooltip=function(){jQuery(".nbd-tooltip-i18n").each(function(){var key=jQuery(this).data("lang");jQuery(this).tooltip({trigger:"hover",title:langjs[key],container:"body"}).on("click",function(){$(this).tooltip("hide")})})};