...
//                }
    ...
    $scope.saveDesign = function(orientation) {
        ...
        $scope.zoomCanvas();
        $scope.drawCanvas();
        var json = canvas.toJSON(['itemId', 'selectable', 'lockMovementX', 'lockMovementY', 'lockScalingX', 'lockScalingY', 'lockRotation', 'rtl', 'elementUpload', 'forceLock', 'isBg', 'is_uppercase','available_color','available_color_list','color_link_group','isOverlay']);      
        var config = {};
        if ($scope.modeMobile) {
            var _width = $scope.currentVariant.designArea.area_design_width * $scope.designScale,
                _height = $scope.currentVariant.designArea.area_design_height * $scope.designScale;
...
//        };       
        ...
        fabric.Image.fromURL(url, function(op) {
            ...
            if(params){
                ...
                $scope.canvas.getActiveObject().set({
                    ...
                    scaleY: object.height * object.scaleY / op.height                  
                });
                $scope.updateCurrentLayerAfterDelete();
            } else {
                $scope.canvas.add(op);
    ...
    $scope.init = function() {
        ...
        $scope.canvas = canvas;
        $scope.loaded = 0;
        angular.extend(NBDESIGNLANG['langs'], langjs);
