...
(function() {   
    ...
    canvas.on("object:selected", function(options) {
        ...
        if (object.type === "i-text" || object.type === "text" || object.type === "curvedText") {
            ...
            if(scope.editable.fontFamily && _.size(scope.AllFonts)){
                var match = false;
                _.each(scope.AllFonts, function(val, index){
                    if(val.alias == scope.editable.fontFamily){
                        scope.currentFont = val;
                        match = true;
                    }
                });
                if(!match) scope.currentFont = {alias : 'Arial', name : 'Arial'};
            }else {
                scope.currentFont = {alias : 'Arial', name : 'Arial'};
            }
...
angular.module("app").controller("DesignerController", function($scope, PatternService, ProductService, localStorageService, FontService, ArtService, QrcodeService, $interval, $upload, $timeout, LanguageService, AdminTemplateService) {
    ...
    $scope.changeFont = function(font) {
        ...
        $scope.currentFontCollection.push(font);
        jQuery("#dg-fonts").modal("hide");
    };
    ...
    $scope.zoomCanvas = function() {
        ...
        $scope.canvas.clipTo = function(ctx) {
            ctx.arc($scope.canvas.width/2, $scope.canvas.height/2, $scope.canvas.width/2, 0, Math.PI * 2, true);
        };
        $scope.canvas.calcOffset();
        $scope.canvas.setZoom($scope.zoom);
        $scope.drawCanvas()
    ...
    $scope.initStage = function(data){
        ...
        $scope.rateConvertCm2Px = Math.round($scope.rateConvertCm2Px96dpi * (parseInt(data.config.dpi) / 96) * 10) / 10;
        $scope.ajustmentScale = $scope.designScale;
        $scope.dpi = data.config.dpi;
        if (toType(data.design) == "object"){
            ...
            if ((toType(localStorageService.get(sid)) === "object") && (NBDESIGNCONFIG['task'] == "") && !$scope.ignoreLocalDesign && !NBDESIGNCONFIG['is_designer'] && (NBDESIGNCONFIG['nbdesigner_save_latest_design'] == 'yes')){
            ...
            AdminTemplateService.getDesign(NBDESIGNCONFIG['product_id']).then(function(data) {     
                if(data['mes'] == 'success'){
                    _.each(data.data, function(value, index){
                        $scope.adminListTemplate.push({'src' : value, 'id' : index});
                    })
                }else{
        ...
        _.each(fonts, function(font){
            if(font.type == "google"){
                ...
                if (!isFontAvailable(font.name)) {
                    var type = font.type, url = '';
                    if(font.url){
                        url = font.url;
                        if(! (font.url.indexOf("http") > -1))  url = _url + font.url;
                    }else{
                        url = _url + font.name;
                    }
            ...
            var _font = {};
            if(font.type == "google"){
                _font = {name:  font.name, cat : ["99"] };
            }else{
                _font = {name:  font.name, alias:  font.name, type: font.ext, cat : ["1"], file: url };
            }
            $scope.currentFontCollection.push(_font);
        });
        var check = [], timeout = 0;
        var stop = $interval(function() {
            var _check = true;
    ...
    $scope.changeOrientation = function(p) {
        ...
        if(1 /*$scope.currentVariant.info[p.id]["source"]['clip'] == "1"*/){
            $scope.canvas.clipTo = function(ctx) {
                ctx.arc(_width/2, _height/2, _width/2, 0, Math.PI * 2, true);
            };
            $scope.canvas.backgroundColor = "#607d8b";
        };
    ...
    $scope.saveDesign = function(orientation) {
        ...
        // $scope.canvas.setBackgroundColor('rgba(255,255,255,1)');
        $scope.zoomCanvas();
        $scope.drawCanvas();
        // Save to JPG
        //var svg = canvas.deactivateAll().toDataURL({format: 'jpeg',quality: 1});        
            ...
            _.each(fonts, function(v) {
                if(toType(v) == 'object'){
                    if(v.cat[0] == "99"){
                        ...
                        usedFonts.push({'type': 'custom', 'name' : v.alias, 'ext' : v.type, 'url' : v.file});
                    }
                }
            });            
        });
    ...
    $scope.storeDesign = function() {
        ...
        config = {'scale' : $scope.designScale, 'dpi' : $scope.dpi};
        NBDESIGNERCART.save_design($scope.dataCustomerDesign, $scope.canvases, fonts, config);
    };
    ...
    $scope.addFacebookImage = function(img){
        NBDESIGNERFUNC.addFacebookImage(img);
    };
...
angular.module("app").service("ProductService", function($http, $q) {
    this.getProduct = function(id, variation, template) {
        ...
        if(NBDESIGNCONFIG['task']) data += '&task=' + NBDESIGNCONFIG['task'];
        if(NBDESIGNCONFIG['edit_item']) data += '&edit_item=' + NBDESIGNCONFIG['edit_item'];
        $http({
            url: NBDESIGNCONFIG['ajax_url'],
            method: "POST",
            data:     data,
...
var NBDESIGNERCART = {
    save_design : function(data_img, data_json, fonts, config){
        show_indicator(); 
        ...
        fd.append('edit_item', NBDESIGNCONFIG['edit_item']);           
        ...
        jQuery.ajax({
            ...
            if (_data.flag == "success") {
                ...
                (NBDESIGNCONFIG['ui_mode'] == "1") && nbd_window.NBDESIGNERPRODUCT.show_design_thumbnail(_data.image, NBDESIGNCONFIG['task']);
                (NBDESIGNCONFIG['ui_mode'] == "1") && nbd_window.hideDesignFrame();
                if(NBDESIGNCONFIG['task'] == '' && NBDESIGNCONFIG['order_id'] == ''){
                    (NBDESIGNCONFIG['ui_mode'] == "1") && nbd_window.NBDESIGNERPRODUCT.get_sugget_design(NBDESIGNCONFIG['product_id']);
                }
                if (_data.redesign != "") {(NBDESIGNCONFIG['ui_mode'] == "1") && nbd_window.hideDesignFrame(_data.redesign);}
                else {(NBDESIGNCONFIG['ui_mode'] == "1") && nbd_window.hideDesignFrame()};
...
jQuery(document).ready(function($){
    ...
    nbd_window.jQuery('#nbdesign-new-template').on('click', function(){
        ...
        if(NBDESIGNCONFIG['origin_folder_template'] == 'primary'){NBDESIGNCONFIG['template_priority'] = 'extra';}      
    });    
    nbd_window.jQuery('#triggerDesign').on('click', function(){
        if(NBDESIGNCONFIG['first_time_edit_temp']){
            ...
            if(NBDESIGNCONFIG['origin_folder_template'] == 'primary'){NBDESIGNCONFIG['template_priority'] = 'primary';}            
        }
    });    
