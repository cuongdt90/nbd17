...
//        };
    ...
    $scope.initStage = function(data){
        ...
        if( angular.isDefined(data.option) ) {
            ...
            if( $scope.productOptions.allow_specify_dimension == 1 && $scope.settings.task != 'create'){
    ...
    $scope._changeDimension = function( dim ){
        ...
        $scope.customWidth = dim.width;
        $scope.customHeight = dim.height;
        $scope.customPrice = dim.price;
...
//        };
    ...
    $scope.showBleed = function(){
        if( $scope.currentVariant.info[$scope.currentSide.id].source.show_bleed == 1 ){
            jQuery('.nbd-bleed').toggle();
        }
        if( $scope.currentVariant.info[$scope.currentSide.id].source.show_safe_zone == 1 ){
            jQuery('.nbd-safe-zone').toggle();
        }        
    };
...
angular.module("app").service("ProductService", function($http, $q) {
    this.getProduct = function(template_id) {
        ...
        var data = "action=nbdesigner_get_product_info&nonce=" + NBDESIGNCONFIG['nonce'] + "&product_id=" + NBDESIGNCONFIG['product_id'] + '&variation_id=' + NBDESIGNCONFIG['variation_id'] + '&task=' + NBDESIGNCONFIG['task'];
