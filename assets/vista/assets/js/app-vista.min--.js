...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$timeout', '$http', '$document',
    ...
    $scope.initSettings = function(){
        ...
        $scope.includeExport = ['objectCaching', 'itemId', 'selectable', 'lockMovementX', 'lockMovementY','lockScalingX', 'lockScalingY', 'lockRotation', 'rtl', 'elementUpload', 'forceLock', 'isBg','is_uppercase','available_color','available_color_list','color_link_group','isOverlay', 'isAlwaysOnTop', 'ilr', 'oos'];
        ...
        if(angular.element($event.target).hasClass('stage')){
            ...
            if( $scope.settings.nbdesigner_save_latest_design == 'yes' ){
                ...
                $scope.localStore.update(pid, json, function(){
                    jQuery('.nbd-toasts').nbToasts('All change saved');
                });
            }
        }
    ...
    $scope.getCurrentLayerInfo = function(){
        ...
        if( objects.length > 1 ){
            ...
            if( object ){
                ...
                ['type', 'itemId', 'lockMovementX', 'lockMovementY', 'lockScalingX', 'lockScalingY', 'lockRotation', 'lockUniScaling', 'selectable', 'visible', 'angle'].forEach(function(key){
...
//console.log(_canvas.getActiveObject());
    ...
    $scope.rotateLayer = function(command){
        ...
        switch(command){
            ...
            case 'reflect-ver':
                ...
                break;
            case '90cw':
                // TODO : change function getAngle to get('angle') Fabric V2.3.3
    ...
    $scope.setTextAttribute = function(type, value, extra_option){
        ...
        $scope.beforeObjectModify(item);
        item.set({[type]: value});
        // console.log($scope.stages[$scope.currentStage].states.isGroup);
        ...
        // console.log(_states.text.fontWeight);
        // console.log(_states.text.fontStyle);
        ...
        link: function( scope, element, attrs ) {
            $timeout(function() {
                ...
                el.on('ps-scroll-y', function(){
                    ...
                    if( elInfo.length ){
                        // TODO change class show to nbd-show
