...
//                    }                
    ...
    $scope.calcViewport = function(){
        var _offsetWidth = checkMobileDevice() ? 20 : 90,
...
$scope.groupLayers = function(){
    ...
    $scope.setTextAttribute = function(type, value, extra_option){
        ...
        if (items.length > 1) {
            if (_states.isGroupText || type == 'opacity') {
                _.each(items, function (item, key) {
                    $scope.beforeObjectModify(item);
                    item.set({[type]: value});
                });
...
// console.log(NBDESIGNCONFIG['nbdesigner_enable_facebook_photo']);
// console.log(NBDESIGNCONFIG['fbID']);
