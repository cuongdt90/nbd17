...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$timeout', '$http', '$document',
    ...
    $scope.initSettings = function(){
        ...
        $scope.includeExport = ['objectCaching', 'itemId', 'selectable', 'lockMovementX', 'lockMovementY','lockScalingX', 'lockScalingY', 'lockRotation', 'rtl', 'elementUpload', 'forceLock', 'isBg','is_uppercase','available_color','available_color_list','color_link_group','isOverlay', 'isAlwaysOnTop', 'ilr', 'oos', 'evented'];
    ...
    $scope.init = function() {
        ...
        _document.bind('webkitfullscreenchange mozfullscreenchange fullscreenchange MSFullscreenChange', function(){
            ...
            // Todo : change class fullScreenMode to vista
            // jQuery("body").toggleClass("fullScreenMode");
            jQuery(".nbd-mode-vista").toggleClass("fullScreenMode");
            $timeout(function(){
    ...
    $scope.changeVariation = function(){
        ...
        $scope.saveDesign();
        NBDDataFactory.get('nbdesigner_get_product_info', {product_id: NBDESIGNCONFIG['product_id'], variation_id: NBDESIGNCONFIG['variation_id']}, function(data){
            $scope.toggleStageLoading();
            $scope.onloadVariation = false;
    ...
    $scope.toggleStageLoading = function(){
        ...
        // Todo : change class nbd-onloading of body into seft of nbd-mode-vista
    ...
    $scope.generateTypoLink = function(typo){
        if( $scope.settings.task == 'typography' ){
            ...
            return 'http://media.printcart.com/images/data/typography/store/' + typo.folder + '/preview.png';
        }
    };
    /* Fonts */
        ...
        if(angular.element($event.target).hasClass('stage')){
            ...
            if( $scope.settings.nbdesigner_save_latest_design == 'yes' ){
                ...
                $scope.localStore.update(pid, json, function(){
                    // Todo : add infomation to toast
                    jQuery('.nbd-toasts').nbToasts('All changes saved');
                });     
            }
        }
            ...
            if( !$scope.isTemplateMode ){
                $timeout(function(){
                    if( angular.isDefined($scope.settings.product_data.design) ){
                        var viewport = $scope.settings.product_data.config.viewport;
                        if( angular.isDefined( $scope.settings.product_data.config.scale ) ){
                            viewport = {width: $scope.settings.product_data.config.scale * 500, height: $scope.settings.product_data.config.scale * 500};
                        }
    ...
    $scope.onObjectMoving = function(id, options){
        var item = options.target;
        // vista : hiident toolbox when move object
        $scope.stages[$scope.currentStage].states.isShowToolBox = false;
        $scope.updateAssociateLayer(item); 
    };
    // vista : hiident toolbox when move object
...
//            }
    ...
    // mode vista ~ Set position for toolbox text and image
    $scope.setPositionToolbox = function () {
        ...
        var ratioScale = stage.states.scaleRange[stage.states.currentScaleIndex].ratio,
            paddingL = $stageMainActive.offset().left - $layout.offset().left,
        ...
        pTop = bound.top + paddingT + bound.height + stage.config.top * ratioScale - $stageActive.scrollTop() + 14;
        if ((bound.top + bound.height) > _canvas.height) {
            pTop = _canvas.height + paddingT + stage.config.top * ratioScale - $stageActive.scrollTop() + 7;
        }
        pLeft = bound.left + bound.width / 2 + paddingL + stage.config.left * ratioScale;
        var checkPos = bound.left + bound.width;
        if (bound.left < 0) {
            // object over canvas
            if (checkPos > _canvas.width) {
                pLeft = _canvas.width/2 + paddingL + stage.config.left * ratioScale;
                stage.states.toolboxTriangle = 'center';
            }else if (checkPos < _canvas.width/2) {
                pLeft = paddingL + stage.config.left * ratioScale;
                if ($window.outerWidth >= 575 && $window.outerWidth < 768) {
                    pLeft = 0;
                    transform = 'translateX(0)';
                }
            }else if ((checkPos <= _canvas.width && checkPos >= _canvas.width/2)) {
                if ((bound.left + bound.width/2) < 0) {
                    pLeft = paddingL + stage.config.left * ratioScale;
                    if ($window.outerWidth >= 575 && $window.outerWidth < 768) {
                        pLeft = 0;
                        transform = 'translateX(0)';
            ...
            if (checkPos > _canvas.width) {
                if (bound.left > _canvas.width/2) {
                    pLeft = $layout.width();
                    transform = 'translateX(-100%)';
                    stage.states.toolboxTriangle = 'left';
                }else {
                    if ((bound.left - bound.width/2) < 0) {
                        pLeft = $layout.width();
                        transform = 'translateX(-100%)';
    ...
    $scope.getCurrentLayerInfo = function(){
        ...
        if( objects.length > 1 ){
            ...
            // Todo : update position layer
            // var bound =  object.getBoundingRect();
            // var stage = $scope.stages[$scope.currentStage];
            // stage.states.toolbarStyle = {
            //     top: bound.top + bound.height + stage.config.top * stage.states.scaleRange[stage.states.currentScaleIndex].ratio,
            //     left: bound.left + bound.width / 2 + stage.config.left * stage.states.scaleRange[stage.states.currentScaleIndex].ratio
            // };
    ...
    $scope.insertTemplate = function(local, temp){
        ...
        if( local ){
            ...
            NBDDataFactory.get('nbdesigner_get_product_info', {product_id: NBDESIGNCONFIG['product_id'], variation_id: NBDESIGNCONFIG['variation_id'], template_id: temp.id}, function(data){
                data = JSON.parse(data);
                if (!angular.isUndefined(data.design) && typeof data.design !== null) {
                    $scope.settings.product_data.design = data.design;
                    $scope.stages[$scope.currentStage].states.hasReset = true;
                }
...
//            });
    ...
    $scope.debug = function(){
        var _canvas = $scope.stages[$scope.currentStage]['canvas'];
        var items = _canvas.getObjects(),
            itemText = null;
        var textType = ['i-text', 'textbox', 'text', 'curvedText'];
        _.each(items, function (item, key) {
            if (_.includes(textType, item.type)) {
                itemText = item;
                return false;
            }
        });
        ...
        itemText.hasBorders = true;
        // console.log(itemText._styleProperties);
...
//console.log(_canvas.getActiveObject());
    ...
    $scope.resetLayer = function () {
        ...
        console.log(angular.isDefined(initialItem));
    ...
    // vista
    $scope.scaleLayerImage = function (command, scale) {
        var _stage = this.stages[this.currentStage],
            _canvas = _stage.canvas,
            obj = _canvas.getActiveObject();
        if (!obj) return;
    ...
    $scope.setTextAttribute = function(type, value, extra_option){
        ...
        if( type.indexOf("font") > -1 ){
            ...
            console.log(_states.text.font);
            console.log(_states.text.font.subset);
...
nbdApp.factory('NBDDataFactory', function($http){
    return {
        get : function(action, data, callback) {
            ...
            if( data.type == 'typography' || data.type == 'get_typo' ) url = 'https://media.printcart.com/v1/typo';
