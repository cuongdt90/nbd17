...
<?php
    ...
    if (!empty($_POST)) {
        ...
        $nbdesigner_plugin->nbdesigner_update_json_setting(NBDESIGNER_DATA_DIR . '/colors.json', $data, $id);
    }

    $list_color = $nbdesigner_plugin->nbdesigner_read_json_setting(NBDESIGNER_DATA_DIR . '/colors.json');
    $cat_color = $nbdesigner_plugin->nbdesigner_read_json_setting(NBDESIGNER_DATA_DIR . '/color_cat.json');
    $current_color_cat_id = 0;
    $current_cat_id = (isset($_GET['cat_id'])) ? $_GET['cat_id'] : '';
    if ($current_cat_id !== '') {
        $list_color = array_filter($list_color, function ($val) use ($current_cat_id){
            return in_array($current_cat_id, $val->cat);
        });
    }
        ...
        <form name="post" action="" method="post" enctype="multipart/form-data" autocomplete="off">
            <div class="nbdesigner-content-left postbox">
                <div class="inside">
                    ...
                    <input type="hidden" name="nbdesigner_color_id" value="<?php echo count($list_color); ?>"/>
                    <p class="submit">
            ...
            <div class="nbdesigner-content-side">
                <div class="postbox" style="padding-bottom: 5px;">
                    <h3><?php _e('Group', 'web-to-print-online-designer'); ?><img src="<?php echo NBDESIGNER_PLUGIN_URL . 'assets/images/loading.gif'; ?>" class="nbdesigner_editcat_loading nbdesigner_loaded" style="margin-left: 15px;"/></h3>
                    <div class="inside">
                            ...
                            <?php if(is_array($cat_color) && (count($cat_color) > 0)): ?>
                                <?php foreach($cat_color as $val): ?>
                                    <li id="nbdesigner_cat_color_<?php echo $val->id; ?>" class="nbdesigner_action_delete_color_cat">
                                        <label>
                                            <input value="<?php echo $val->id; ?>" type="checkbox" name="nbdesigner_color_cat[]" <?php if($update && (count($cat_color) > 0 )) if(in_array($val->id, $cat_color)) echo "checked";  ?> />
                                        </label>
        ...
        $(document).ready(function () {
            ...
            NBDESIGNADMIN.add_color_cat = function (e) {
                ...
                $.ajax({
                    url: admin_nbds.url,
                    method: "POST",
                    data: {'action': 'nbdesigner_add_color_cat', 'name': cat_name, 'id': cat_id, 'nonce': admin_nbds.nonce},
                    beforeSend: function () {
            ...
            NBDESIGNADMIN.delete_cat_color = function (e) {
                ...
                swal({
                    ...
                    var data = {'action': 'nbdesigner_delete_color_cat', 'id': index, 'nonce': admin_nbds.nonce};
            ...
            NBDESIGNADMIN.save_cat_color = function (e) {
                var index = $(e).parent().find('input').val(),
                    ...
                    sefl = $(e);
                var data = {'action': 'nbdesigner_add_color_cat', 'id': index, 'name': name, 'nonce': admin_nbds.nonce};
                $.ajax({
                    url: admin_nbds.url,
                    method: "POST",
                    data: data,
...
//                    id = $('input[name="nbdesigner_color_id"]').val();
//                var data = {
//                    'action': 'nbdesigner_save_color',
//                    'id': id,
//                    'name': name,
//                    'hex': hex,
//                    'nonce': admin_nbds.nonce
//                };
