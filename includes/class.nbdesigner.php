...
class Nbdesigner_Plugin {
    ...
    public function woocommerce_hook(){
        ...
        add_action('woocommerce_add_to_cart', array($this, 'set_nbd_session_cart'), 10, 4);
        if( ! is_woo_v3() ){		
    ...
    public static function plugin_activation() {
        ...
        Nbdesigner_IO::mkdir(NBDESIGNER_ADMINDESIGN_DIR);
        Nbdesigner_IO::mkdir(NBDESIGNER_FONT_DIR);
        Nbdesigner_IO::mkdir(NBDESIGNER_ART_DIR);
        Nbdesigner_IO::mkdir(NBDESIGNER_PDF_DIR);  
    ...
    public function nbdesigner_settings(){
        ...
        $tabs = apply_filters('nbdesigner_settings_tabs', array(
            ...
            'log' => '<span class="dashicons dashicons-art"></span> '. __('Logs', 'web-to-print-online-designer'),            
        ));
    ...
    */    
    ...
    public function nbdesigner_detail_order() {
        if(isset($_GET['order_id'])){
            ...
            if(isset($_GET['download-all']) && ($_GET['download-all'] == 'true')){
                ...
                foreach($products AS $order_item_id => $product){                
                    if(wc_get_order_item_meta($order_item_id, '_nbd')){
                        ...
                        $list_images = Nbdesigner_IO::get_list_thumbs(NBDESIGNER_CUSTOMER_DIR .'/'. $nbd_item_key, 1);
            ...
            if(isset($_GET['nbd_item_key'])){
                ...
                $list_design = array();
                $list_pdfs = array();
                $datas = json_decode(file_get_contents($path .'/product.json'));
    ...
    public function nbdesigner_button($att = false) {
        ...
        if ($is_nbdesign) {
            ...
            $button .= '<h4 id="nbdesigner-preview-title" style="display: none;">'.__('Preview your design', 'web-to-print-online-designer').'</h4>' . $div_image;
            $button .= '</div><br />';
    ...
    public function nbdesigner_get_product_info() {
        ...
        $edit_item = (isset($_POST['edit_item']) &&  $_POST['edit_item'] != '') ? $_POST['edit_item'] : '';
        $user_id = (get_current_user_id() > 0) ? get_current_user_id() : session_id();         
        $data = nbd_get_product_info($user_id, $product_id, $variation_id, $task, $reference_product, $template_folder, $order_id, $order_item_folder, $edit_item);  
    ...
    private function store_design_data($nbd_item_key, $data, $product_config, $design_status = 'new'){
        $path = NBDESIGNER_CUSTOMER_DIR . '/' . $nbd_item_key;
        if($design_status == 'edit') Nbdesigner_IO::delete_folder($path);     
        if ( wp_mkdir_p($path) ) {
            file_put_contents($path . '/product.json', json_encode($product_config));
            foreach ($data as $key => $val) {
                if($key == 'design'){
    ...
    private function create_preview_design($path_src, $path_dst, $product_config, $width, $height){
        ...
        foreach ($product_config as $key => $val){
            ...
            if(file_exists($p_img)){
                if($design_format == 'png'){
                    $image_design = NBD_Image::nbdesigner_resize_imagepng($p_img, $val["area_design_width"] * $scale, $val["area_design_height"] * $scale);
                }else{
                    $image_design = NBD_Image::nbdesigner_resize_imagejpg($p_img, $val["area_design_width"] * $scale, $val["area_design_height"] * $scale);
                }
                ...
                if($val["bg_type"] == 'image'){
                    imagecopy($image, $image_product, $val["img_src_left"] * $scale, $val["img_src_top"] * $scale, 0, 0, $val["img_src_width"] * $scale, $val["img_src_height"] * $scale);
                    ...
                    imagefilledrectangle($image, $val["img_src_left"] * $scale, $val["img_src_top"] * $scale, $val["img_src_left"] * $scale + $val["img_src_width"] * $scale, $val["img_src_top"] * $scale + $val["img_src_height"] * $scale, $color);
                } else if($design_format == 'jpeg') {
                    $color = imagecolorallocate($image, 255, 255, 255);
                    imagefilledrectangle($image, $val["img_src_left"] * $scale, $val["img_src_top"] * $scale, $val["img_src_left"] * $scale + $val["img_src_width"] * $scale, $val["img_src_top"] * $scale + $val["img_src_height"] * $scale, $color);                    
                }
                imagecopy($image, $image_design, $val["area_design_left"] * $scale, $val["area_design_top"] * $scale, 0, 0, $val["area_design_width"] * $scale, $val["area_design_height"] * $scale);
                if($design_format == 'png'){
    ...
    public function nbd_save_customer_design(){
        ...
        $link = $this->store_design_data($nbd_item_key, $_FILES, $product_config, 'new');     
        $path = NBDESIGNER_CUSTOMER_DIR . '/' . $nbd_item_key;
        ...
        if(false != $link){
            $this->create_preview_design($path, $path.'/preview', $product_config, 200, 200);
            $result['image'] = array();
            $images = Nbdesigner_IO::get_list_thumbs($path.'/preview', 1);
    ...
    public function nbd_save_customer_design1(){   
        if (!wp_verify_nonce($_POST['nonce'], 'save-design')) {
            die('Security error');
        }     
        $result = array();
        ...
        $variation_id = (isset($_POST['variation_id']) && $_POST['variation_id'] != '') ? absint($_POST['variation_id']) : 0;     
        if(!$product_id) {
            $result['mes'] = __('Not a product', 'web-to-print-online-designer'); echo json_encode($result); wp_die();        
        }
        $user_id = (get_current_user_id() > 0) ? get_current_user_id() : session_id();
        $task = (isset($_POST['task']) && $_POST['task'] != '') ? $_POST['task'] : '';
        $order_item_folder = (isset($_POST['order_item_folder']) && $_POST['order_item_folder'] != '') ? $_POST['order_item_folder'] : '';
        ...
        if($task == 'redesign'){
            if(!get_current_user_id()) {
                $result['mes'] = __('You must login to re-design', 'web-to-print-online-designer'); echo json_encode($result); wp_die();
            }
        ...
        }else if($task == 'edit_design' || $task == 'design'){
        ...
        if (!count($data_after_save_image['mes'])) {              
            ...
            if(($task == 'create_template' || $task == 'edit_template')){
                $this->nbdesigner_create_thumbnail_design($path, $path.'/preview', $product_id, $variation_id, 500, 500);               
                if(!$save_status){
                ...
                if(wp_mkdir_p($path_old)){
                    ...
                    $result['redesign'] = __("Your design has been saved success! Please wait response email!", 'web-to-print-online-designer');
                    update_post_meta($order_id, '_nbdesigner_order_changed', 1);                     
                }
            }else if($task == 'edit_design' ) {
                $_SESSION['nbdesigner'][$edit_item] = json_encode(Nbdesigner_IO::get_list_thumbs($path . '/thumbs'));
                WC()->session->set($edit_item . '_nbd_path', json_encode(Nbdesigner_IO::get_list_thumbs($path . '/preview')));
            }else if($task == 'design'){
                /* Design in custom page */
                $this->nbdesigner_create_thumbnail_design($path, $path.'/preview', $product_id, $variation_id, 300, 300);      
                WC()->session->set($edit_item . '_nbd_path', json_encode(Nbdesigner_IO::get_list_thumbs($path . '/preview')));
                WC()->session->set($edit_item . '_nbdesigner', 'has_design');
            }else{   
                /* Design in product page */
                $this->nbdesigner_create_thumbnail_design($path, $path.'/preview', $product_id, $variation_id, 300, 300);      
                $_SESSION['nbdesigner']['nbdesigner_' . $product_id] = json_encode(Nbdesigner_IO::get_list_thumbs($path . '/preview'));
                $nbd_item_key = ($variation_id > 0) ? $product_id . '_' . $variation_id : $product_id; 
                WC()->session->set('nbd_item_key_'.$nbd_item_key, json_encode(Nbdesigner_IO::get_list_thumbs($path . '/preview')));
            }          
        }else{
  ...
  PRIMARY KEY  (id)
    ...
    public function nbdesigner_save_design_to_image2($data, $path, $pid, $vid) {
        ...
        if (!file_exists($path)) {
            if (wp_mkdir_p($path)) {
                if (!file_exists($path_thumb))
                    if (!wp_mkdir_p($path_thumb)) {
                        $mes[] = __('Your server not allow creat folder', 'web-to-print-online-designer');
                    }
            } else {
                $mes[] = __('Your server not allow creat folder', 'web-to-print-online-designer');
            }
        ...
        foreach ($data as $key => $val) {
            ...
            if (move_uploaded_file($val["tmp_name"],$full_name)) {      
                ...
                $mes[] = __('Your server not allow writable file', 'web-to-print-online-designer');
            }                
        }
    ...
    public function nbdesigner_render_cart($title = null, $cart_item = null, $cart_item_key = null) {
        if ($cart_item_key && is_cart()) {
            ...
            if (isset($nbd_session)) {
                $html = $title . '<div style="display: block;">';
                $list = Nbdesigner_IO::get_list_thumbs(NBDESIGNER_CUSTOMER_DIR . '/' . $nbd_session . '/preview');
                foreach ($list as $img) {
                    $src = Nbdesigner_IO::convert_path_to_url($img);
                    $html .= '<img style="max-width: 100%; display: inline-block; margin: 15px; border: 1px solid #ddd;" src="' . $src . '"/>';
                }
    ...
    private function nbdesigner_change_folder_after_add_to_cart($cart_item_key, $product_id){ 
        ...
        $sid = session_id();
        $iid = ($uid > 0) ? $uid : $sid;
        $old_path = NBDESIGNER_DATA_DIR . '/designs/' . $iid . '/nb_order/'.$product_id;
        $new_path = NBDESIGNER_DATA_DIR . '/designs/' . $iid . '/nb_order/'.$cart_item_key;
...
error_log(ob_get_clean());
    ...
    public function order_item_name($item_name, $item){   
        ...
        if(isset($item["item_meta"]["_nbd"]) && $show_design_in_order == 'yes'){
            $nbd_item_key = $item["item_meta"]["_nbd"]; 
            $design_data = unserialize(get_post_meta($item['order_id'], '_nbdesigner_design_file', true));
            $images = Nbdesigner_IO::get_list_thumbs(NBDESIGNER_CUSTOMER_DIR .'/'. $nbd_item_key .'/preview', 1);            
            $notice = '';
            $html = '<div>';
            foreach ($images as $img){      
                $src = Nbdesigner_IO::convert_path_to_url($img);     
                $html .= '<img style="max-width: 100%; border: 1px solid #ddd; display: inline-block; margin: 15px;" src="'.$src.'" />';
            }
            $html .= '<div>';
            $design_link = getUrlPageNBD('redesign');
    ...
    public function nbdesigner_order_item_quantity_html($strong, $item){
        ...
        if($show_design_in_order == 'yes'){
            if(isset($item["item_meta"]["_nbd"]))  return '';
        }
	...
	global $wpdb;
	$redesign_page_id = $wpdb->get_var("SELECT ID FROM $wpdb->posts WHERE post_name='".NBDESIGNER_PAGE_REDESIGN."' AND post_status NOT IN ( 'pending', 'trash', 'future', 'auto-draft' )");	        
        if ($redesign_page_id == ''){        
            $post = array(
                'post_name' => NBDESIGNER_PAGE_REDESIGN,
                'post_status' => 'publish',
                'post_title' => __('Customer re-design product', 'web-to-print-online-designer'),
                'post_type' => 'page',
                'post_author'    => 1,
                'comment_status' => 'closed',
                'post_date' => date('Y-m-d H:i:s')
            );      
            $redesign_page_id = wp_insert_post($post, false);	
        }
        update_option( 'nbdesigner_redesign_page_id', $redesign_page_id );
        $template_page_id = $wpdb->get_var("SELECT ID FROM $wpdb->posts WHERE post_name='".NBDESIGNER_PAGE_CREATE_TEMPLATE."' AND post_status NOT IN ( 'pending', 'trash', 'future', 'auto-draft' )");
        if ($template_page_id == ''){        
            $post = array(
                'post_name' => NBDESIGNER_PAGE_CREATE_TEMPLATE,
                'post_status' => 'publish',
                'post_title' => __('Create Template', 'web-to-print-online-designer'),
                'post_type' => 'page',
                'post_author'    => 1,
                'comment_status' => 'closed',
                'post_date' => date('Y-m-d H:i:s')
            );      
            $template_page_id = wp_insert_post($post, false);	
        }  
        update_option( 'nbdesigner_template_page_id', $template_page_id );
        $studio_page_id = $wpdb->get_var("SELECT ID FROM $wpdb->posts WHERE post_name='".NBDESIGNER_PAGE_STUDIO."' AND post_status NOT IN ( 'pending', 'trash', 'future', 'auto-draft' )");
        if ($studio_page_id == ''){        
            $post = array(
        ...
        $create_your_own_page_id = $wpdb->get_var("SELECT ID FROM $wpdb->posts WHERE post_name='".NBDESIGNER_PAGE_CREATE_YOUR_OWN."' AND post_status NOT IN ( 'pending', 'trash', 'future', 'auto-draft' )");
        if ($create_your_own_page_id == ''){        
            $post = array(
    ...
    public function nbdesigner_redesign_func(){       
        ...
        $variation_id = (isset($_GET['variation_id']) && $_GET['variation_id'] != '') ? $_GET['variation_id'] : '';
        ...
        $path = NBDESIGNER_DATA_DIR . '/designs/' . $uid . '/' . $order_id . '/' . $folder . '/thumbs';
        $list_image = $this->nbdesigner_list_thumb($path);
        if (count($list_image) > 0) {
            $div_image = '<div id="nbdesigner_frontend_area"><h4>' . __('Preview your design', 'web-to-print-online-designer') . '</h4>';
        ...
        $html .= '<a class="button nbdesign-button nbdesigner-disable" id="triggerDesign" >' . '<img class="nbdesigner-img-loading rotating" src="'.NBDESIGNER_PLUGIN_URL.'assets/images/loading.png'.'"/>'.__('Design again', 'web-to-print-online-designer').'</a><br />' . $div_image;
        $html .= '</div><br />';  
    ...
    public function nbdesigner_admindesign_func(){
        ...
        if(!($uid > 0) || !isset($product_id) || !(current_user_can('edit_nbd_template'))) return $html = __("Oops! you can't accsess this page!", 'web-to-print-online-designer');
    ...
    public function nbdesigner_get_suggest_design(){
        ...
        if(isset($_POST['ref']) && isset($_POST['products']) && isset($_POST['sid'])){    
            ...
            $from_path = NBDESIGNER_DATA_DIR . '/designs/' . $iid . '/nb_order/' . $ref_pid;
            if(is_array($products)){
                foreach ($products as $pro){
                    $folder = substr(md5(rand(0, 999999)), 0, 10);
                    $path_suggest = NBDESIGNER_DATA_DIR . '/suggest_designs/' . $folder;
                    $this->nbdesigner_create_thumbnail_design($from_path, $path_suggest, $pro, 0, 500, 500);
    ...
    public function nbdesigner_save_design_to_pdf(){       
        ...
        foreach($pdfs as $key => $_pdf){
            ...
            if($customer_design != ''){
                $path_cd = wp_make_link_relative( $path_cd );
            }            
            if($pdf_format == '-1'){
                $pWidth = $bgWidth + $mLeft + $mRight;
                $pHeight = $bgHeight + $mTop + $mBottom;
            ...
            if(!$force){
                ...
                $output_file = $folder .'/'. $key .$_pdf['name'].'.pdf';
